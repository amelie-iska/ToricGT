...
def test_default_graph_of_thought_template_branches_and_merges():
    ...
    assert summary["branch_merge_edge_fraction"] > 0.75
    assert summary["linear_chain_fraction"] < 0.25
...
def test_got_dag_metrics_are_finite_for_branching_hidden_states():
    ...
    assert out["got_dag_branch_merge_edge_fraction"] > out["got_dag_linear_chain_fraction"]


def test_linear_chain_is_detected_as_chain_collapse():
    hidden = torch.randn(1, 8, 16)
    node_mask = torch.ones(1, 8, dtype=torch.bool)
    edge_index = torch.tensor([[[idx, idx + 1] for idx in range(7)]], dtype=torch.long)
    edge_mask = torch.ones(1, 7, dtype=torch.bool)

    out = got_dag_metrics(hidden, node_mask=node_mask, edge_index=edge_index, edge_mask=edge_mask)

    assert out["got_dag_linear_chain_fraction"] > 0.99
    assert out["got_dag_branch_merge_edge_fraction"] < 0.01
