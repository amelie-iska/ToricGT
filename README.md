...
**Recommendation for Newcomers to Algebraic Geometry of any kind:** *if you are a complete beginner with algebraic geometry, you may be surprised by the theory and techniques; they are much different from the basic differential geometry needed for deep learning and AI applied to scientific problems and you may find yourself feeling in over your head early on; for such readers and researchers more comfortable with differentiable probability theory inherent in pretty much every backprop based neural network, i recommend giving [Karen Smith's Book "An Invitation to Algebraic Geometry"](./assets/an-invitation-to-algebraic-geometry.pdf) a read; that book is where i started when i began learning, along with the beloved Hartshorne, and the various Eisenbud companion books on the topic, and i became something of an expert in no time; Karen's book will not give a deep, comprehensive, advanced and technical theoretical reference to the whole area the way Hartshorne and Eisenbud's books will; what it will do is make you feel like everything in those books is accessible from both a conceptual and a theoretical perspective, and it will make you feel like that initial learning curve has a far gentler derivative; after Karen's book, i suggest the following:*

- [Combinatorial Commutative Algebra (Toric Section)](./assets/combinatorial-commutative-algebra.pdf) 
- [Toric Varieties (thorough reference book)](./assets/toric-varieties.pdf) 
 
*then i would work through Fulton's books in whatever order you like along with the tropical intersection theory book* 
...
`data/uniprot_fot/derived`, leakage-aware split shards live in
`data/uniprot_fot/splits`, and coordinate-native structure shards live under
`data/uniprot_fot/structures`.  The published dataset is
[`AmelieSchreiber/toricblm_fot`](https://huggingface.co/datasets/AmelieSchreiber/toricblm_fot).
The current full-training gate uses
`data/uniprot_fot/manifests/toricblm_multimodal_structure_readiness.json`,
which must pass before the launcher starts training.  The July 2026 large-data
gate waits for capped/diverse AFDB-GCS protein structures, local full-PDB
complex curation, PubChem3D conformers, a strict GPU ProTrek/Foldseek trimodal
split for rows with sequence, function, and structure, and a training-corpus
audit report.  Missing structures do not exclude useful examples: rows with
sequence plus function annotations but no structure remain trainable in the
graph/FoT streams and are skipped only by coordinate-specific losses.  The
launcher writes `toricblm_training_corpus_audit.json`, including a
`sequence_function_without_structure_trainable` count, before training.

The ProTrek/Foldseek-backed splitter is
`scripts/split_with_protrek_trimodal_streaming.py`.  It runs actual GPU
ProTrek inference over sequence, function text, and Foldseek/3Di structure
representations, clusters only true trimodal rows, writes full train,
validation, and test Parquet shards, and leaves sequence/function-only rows in
the ordinary graph/FoT corpus.  The strict structure-flow training path consumes
the resulting `data/uniprot_fot/splits/protrek_structure_current/train/*.parquet`
shards rather than unsplit AFDB accession hashes.
...
Long graph/FoT entries now have a row-preserving training path instead of being
reduced to anonymous 1024-token chunks.  Set `LONG_ENTRY_TRAINING=1` to use
`FullEntryGraphParquetStream`, which keeps each graphified UniProt/FoT row as a
single training object, tokenizes the full row, and segments it across the
entire record up to `LONG_ENTRY_MAX_TOKENS`.  The active config prioritizes the
ProTrek/UniProt FoT train shard, uses `LONG_ENTRY_MAX_TOKENS=24576`,
`LONG_ENTRY_SEGMENT_LEN=1024`, `LONG_ENTRY_MAX_SEGMENTS=0`, and trains every
segment in the row with local causal BPB loss plus next-segment contrastive
prediction, segment-boundary continuity, and a coverage anti-collapse term.
In addition, `LONG_ENTRY_FULL_CONTEXT_EVERY=32` runs a sparse dense
full-context pass over up to `8192` tokens from a long row with activation
checkpointing enabled.  This is the practical ToricGT/ToricBLM long-context
route: segmented full-row graph training supplies full-record coverage, while
periodic dense passes teach genuinely long causal attention without making
the whole run memory-bound.  `LONG_ENTRY_FOT_LOSS_WEIGHT` additionally applies
the same embedding-space Forest-of-Thought head to selected long graph/FoT
segments, so biomedical rows directly train sparse forest activation, UCB-style
branch selection, self-correction, trajectory balance, and consensus without
requiring a structure field to be present.
...
FoT now has both training and inference/reporting support:
`src/toricgt/embedding_forest_of_thought.py` emits explicit forest-root,
tree-expansion, self-correction, and consensus-vote edges, and
`scripts/export_oai_fot_trace.py` exports those traces from checkpoints or in
smoke-test mode for visualization and audit.
