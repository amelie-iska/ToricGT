...
which must pass before the launcher starts training.  The current manifest is
ready and contains real coordinate-bearing train rows for all required
modalities: AFDB protein, PDB protein, PDB RNA, PDB DNA, PDB mixed
protein/nucleic-acid complexes, and PubChem3D/ligand conformers.  Missing
structures remain trainable in the graph/FoT stream and are skipped only by
coordinate-specific losses.

The current ProTrek/Foldseek-backed split report is
`data/uniprot_fot/manifests/toricblm_fot_protrek_split_report_v2.json`.
It was generated by actual GPU ProTrek inference over the populated
graph/structure corpus, not by hash-only signatures: 14,411 rows were embedded,
13,227 were assigned to train, 373 to validation, and 811 to test over 120
clusters.  The split report records 4,335 trimodal rows with sequence, function
text, and structure present.  AFDB curation remains resumable and may continue
to add shards under `data/uniprot_fot/structures/afdb_v6_full`; after materially
larger additions, rerun the readiness manifest and ProTrek split before a final
large training restart.
...
every step quadratic over 10k--20k tokens.
