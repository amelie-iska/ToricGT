...
\[
  ...
  e_{J\setminus\{j_r\}}.
\]
This Taylor complex is a full symbolic multigraded free resolution, while the Hochster table gives the minimal Betti numbers, projective dimension, regularity, and nonminimality gap.
...
\]
the Stanley--Reisner generator loss is $\mean_{F\in\mathcal G(I_\Delta)}\prod_{i\in F}x_i(H)$, Taylor lcm-syzygy mass is the corresponding average over $\operatorname{lcm}(m_a,m_b)$, and the Hilbert/Betti pressure multiplies these polynomial maps by constants derived from the exact Betti table, projective dimension, and regularity.  Thus the symbolic multigraded ring is used exactly for audits and differentiably for optimization.
