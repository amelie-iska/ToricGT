...
The competition branch adds an auxiliary training signal inspired by graph contrastive geometry: hidden transitions should form reusable analogical arrows rather than unrelated local displacements.  This signal is deliberately auxiliary.  It does not replace BPB, and its weights are phased in only after the early byte compressor has a stable negative slope.

Let $h_t\in\R^d$ be a prefix-visible hidden state and choose a short stride $s$.  Define a normalized relation arrow
\begin{equation}
  a_t=\frac{h_{t+s}-h_t}{\norm{h_{t+s}-h_t}_2+\epsilon}.
\end{equation}
Coarse byte-relation classes are obtained from source and target byte classes: for example digit-to-digit, lowercase-to-punctuation, or Hebrew-byte-to-Hebrew-byte.  If two arrows belong to the same class, they should represent the same abstract operation up to local context.  The GraphCG-style basis $B\in\R^{d\times r}$ supplies a compact lattice chart:
\begin{equation}
  c_t=\softmax(B\transpose a_t/\tau),\qquad
  \widehat a_t=Bc_t.
\end{equation}
The basis loss combines reconstruction, entropy control, and margin terms.  An analogical functor loss reduces within-class variance of $a_t$, while a parallelogram loss penalizes failures of local closure:
\begin{equation}
  \mathcal L_{\mathrm{para}}
  =\Eop\norm{(h_{t+s}-h_t)+(h_{u+s}-h_u)-(h_{v+s}-h_v)}_2^2
\end{equation}
over adjacent relation triples that share the same coarse arrow label.  These terms encourage byte-level reasoning moves to become reusable lattice vectors.
...
Periodic analyses render these quantities directly.  For selected reasoning records the script writes directed filtration curves for edge density, triangle density, asymmetry, and cycle flux, plus heatmaps of normalized distances, the antisymmetric toric skew matrix, and directed adjacencies at representative radii.  Thus the noncommutative topology is both a training pressure and an auditable explanation object.
