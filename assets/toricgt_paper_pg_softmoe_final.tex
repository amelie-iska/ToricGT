...
\begin{figure}[p]
  \centering
  \includegraphics[width=\textwidth]{toricgt_architecture_and_training_diagram.pdf}
...
\begin{enumerate}[leftmargin=2em]
  \item a byte-level or small-BPE tokenizer, with tied input/output embeddings;
  \item a small number of unique Transformer blocks reused recurrently to increase effective depth without storing more block parameters;
  \item lower dense blocks for stable language modeling and upper \emph{prefix-causal} Soft-MoE blocks with very small $E$ and $S$;
  \item tiny toric/tropical adapters that add phase-like and max-gated residuals without large parameter cost;
  \item quantization-aware training and final int6/int8 packing plus zlib/zstd round-trip checks;
  \item optional legal score-first test-time adaptation only on validation tokens already scored, never on future validation tokens.
\end{enumerate}

A safe starting microarchitecture is
\begin{equation}
  V=1024\text{ or }2048,\\quad d=384\text{ or }512,
  \quad L_{\mathrm{unique}}=4\text{--}6,
  \quad L_{\mathrm{effective}}=8\text{--}12,
\end{equation}
with grouped-query attention, tied embeddings, recurrent block tying, and Soft-MoE only in the last two unique blocks.  If $P_{\mathrm{fp}}$ is the uncompressed scalar count, then int8 export roughly permits $P_{\mathrm{fp}}<16$M minus code overhead; int6 export roughly permits $P_{\mathrm{fp}}<21$M before entropy compression; int4 export permits more parameters but is less stable unless quantization-aware training is strong.  These are engineering estimates; the actual gate is the serialized artifact size.
...
python train_gpt.py --smoke_train --max_steps 20
...
\end{verbatim}
Run a bounded smoke curation with:
\begin{verbatim}
conda run -n tokengt env PYTHONPATH=src python scripts/curate_datasets.py \
...
\begin{enumerate}[leftmargin=2em]
  \item \textbf{Phase A: smoke tests}.  CPU-only shape tests, equivariance tests, mask tests, and exact tropical ring-vs-full comparisons.
  ...
  \item \textbf{Phase F: Parameter-Golf distillation and compression}.  Distill graph-reasoning behavior into a micro language model, add compact Soft-MoE adapters only after a dense baseline is stable, run quantization-aware training, and export a self-contained artifact with exact byte accounting.
\end{enumerate}

\section{Evaluation and ablations}

Core metrics include node accuracy/regression, edge accuracy/regression, graph answer exact match, proof validity, equivariance error, tropical margin and active-face entropy, torus commutator error, Hebrew root/binyan/feature F1, paradigm consistency, Soft-MoE expert utilization and slot entropy, Parameter-Golf artifact bytes and validation BPB for the micro-LM track, GFlowNet reward calibration, terminal diversity, and OOD generalization over graph size, word length, denominator $q$, Coxeter rank, and Hebrew root families.
...
\begin{enumerate}[leftmargin=2em]
  ...
  \item \texttt{causal\_soft\_moe.py}: prefix-scan Soft-MoE for autoregressive Parameter-Golf scoring, causal audits, score-before-update tests.
...
\midrule
\endhead
Smoke model & 2 layers, $d=64$ & CPU-only shape and equivariance tests \\
...
Soft-MoE Parameter-Golf & $E=2$--$4$, $S=1$--$2$ & use shared low-rank adapters and QAT; measure bytes after every change \\
Parameter-Golf micro-LM & tied embeddings, recurrent 4--6 unique blocks & target under 16,000,000 artifact bytes with train/eval logs \\
