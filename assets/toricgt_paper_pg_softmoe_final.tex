...
\begin{figure}[t]
  \centering
  \includegraphics[width=0.82\textwidth]{fig_oai_step2250_reasoning_k_bpb.png}
...
\end{split}
\end{equation}
The code path is \texttt{src/toricgt/combinatorial\_toric\_metrics.py}; it returns \texttt{toric\_cca\_topology\_loss} and detached diagnostics such as \texttt{toric\_cca\_binomial\_residual}, \texttt{toric\_cca\_stanley\_reisner\_nonface\_mass}, \texttt{toric\_cca\_betti0\_proxy}, and \texttt{toric\_cca\_betti1\_proxy}.  These diagnostics are useful only under an audit-to-loss rule: a metric may affect gradients only after exact finite audits agree with its differentiable surrogate and matched validation shows no BPB regression beyond tolerance.  This keeps the commutative algebra rigorous without letting it overpower the byte-level objective.
