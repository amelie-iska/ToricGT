...
\end{equation}
with tied embeddings, recurrent block tying, hybrid softmax/tropical-ring attention, and no stored expert bank.  In the current artifact audit this gives about $12.9$M stored scalars and an int8 compressed artifact around $12.8$MB, leaving margin below $16{,}000{,}000$ bytes for code and metadata.  If $P_{\mathrm{fp}}$ is the uncompressed scalar count, then int8 export roughly permits $P_{\mathrm{fp}}<16$M minus code overhead; int6 export roughly permits $P_{\mathrm{fp}}<21$M before entropy compression; int4 export permits more parameters but is less stable unless quantization-aware training is strong.  These are engineering estimates; the actual gate is the serialized artifact size.
...
\begin{enumerate}[leftmargin=2em]
  ...
  \item \texttt{random\_order\_lm.py}: dense random-order autoregressive byte model, toric position phases, tied embeddings, recurrence, and score-before-update tests.
...
Dense Parameter-Golf & $d=384$, 7 unique blocks, 2 recurrent passes & random-order byte AR, hybrid attention, tied embeddings, target under 15.6MB artifact bytes \\
