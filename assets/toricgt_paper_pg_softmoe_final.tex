...
\end{equation}
This is not true Kolmogorov complexity, which is uncomputable; it is an auditable estimator whose compressor and helper context are always named.
...
\end{equation}
Large $S_c$ indicates that the chosen compressor/helper encoding may be asymmetric for the domain at hand.  In the current branch these quantities are diagnostics and checkpoint-selection aids, not replacements for the BPB objective.
...
\begin{equation}
  \lambda_{\mathrm{GFN}}=\lambda_{\mathrm{toric}}=\lambda_{\mathrm{Koszul}}=
...
\end{equation}
with a slightly increased learning-rate multiplier and tighter gradient clipping.  This is the phase used after checkpoint \texttt{random\_order\_step\_00036750.pt}.  The training loop resumed from \texttt{random\_order\_step\_00037250.pt} after a checkpoint-gated analysis pass; the next watcher is scheduled to pause only after a fresh step-$37750$ checkpoint, run the full metric/simplex/geometry suite, and hand off a written recommendation before any further restart.

\begin{figure}[t]
  \centering
  \includegraphics[width=0.82\textwidth]{fig_oai_step37250_reasoning_k_bpb.png}
  \caption{Step-$37250$ \texttt{oai} recovery diagnostic.  The reasoning/K$(x\mid\mathrm{helpers})$/BPB simplex places graph-of-thought branches in a three-way tradeoff among additional reasoning budget, conditional-compression structure, and low BPB.  The plot is produced from model computations by the periodic analysis suite; it is an audit snapshot, not a final benchmark claim.}
  \label{fig:oai-step37250-simplex}
\end{figure}

The most recent completed analysis gate at step $37250$ used four held-out validation records and $24$ GFlowNet branches.  It reported mean branch BPB $4.758$, best branch BPB $4.039$, mean answer-span BPB $4.644$, best answer-span BPB $3.630$, mean hidden-trajectory MST efficiency $0.507$, mean path smoothness $0.0895$, directed topology asymmetry $0.401$, HDBSCAN stability $0.943$, exact directed-edge validity $0.958$, exact triangle validity $0.818$, Slepian concentration $1.000$, and Slepian leakage $0.000$.  These values show that the analysis stack is functioning and that graph-of-thought search can find better local branches than the mean branch, but they also show that the BPB objective is still far from the competition target band.  The same pass recorded weak tropical active-face margins, with mean toric-shadow margin around $2.94\times10^{-2}$ and very small minimum margin, so strong toric or Koszul loss should remain off during BPB recovery.  In operational terms, this means the geometry is useful as an audit and routing signal, while byte likelihood must keep priority until the BPB descent slope is restored.
...
The practical antidote to overfitting the theory is a hard gate: no advanced geometric object changes training unless it has a logged metric, an ablation, and a plot path.  The named ``there be dragons'' onlooker is not a model component, prompt role, reward term, or hidden observer; repository search verifies it is absent from executable paths outside ignored output/checkpoint/data directories.  More importantly, the scientific guardrail is positive rather than rhetorical:
\begin{enumerate}[leftmargin=2em]
