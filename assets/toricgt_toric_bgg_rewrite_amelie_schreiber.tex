...
SP1024 baseline tokenizer or the ConvexTok-2048 Det tokenizer.  The current
ConvexTok restart uses $V=2048$, $d=512$, $H=8$, $L=9$, tied embeddings, RoPE
with \texttt{ROPE\_BASE}=10000, sequence length $1024$, and a training-token
batch near $10^6$ tokens to use roughly the 20GB VRAM budget.  With first-class
FineWeb TokenGT identifiers and graph-output flattening enabled, the current
configuration has about $18.09$M trainable parameters before sidecar heads.
