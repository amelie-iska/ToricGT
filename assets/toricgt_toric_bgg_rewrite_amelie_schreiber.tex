...
\end{equation}
This conversion is valid as a diagnostic normalization for the graph-node
head, and it is often more interpretable than raw SP1024 bits/token.  It is
still marked as estimated because the model being scored is not emitting a
byte-level next-symbol distribution over the same prequential process.  Thus
dashboards must display all three concepts under different names:
\[
\operatorname{BPB}_{\mathrm{OAI,official}},\qquad
\operatorname{BPT}_{\mathrm{SP1024,TokenGT}},\qquad
\widehat{\operatorname{BPB}}_{\mathrm{SP1024}\to\mathrm{byte}}.
\]
Only the first should decide Parameter-Golf success; the latter two guide
TokenGT training and ablations.
...
Parameter-Golf BPB claim.  Official byte BPB, TokenGT SP1024 bits/token, and
SP1024-to-byte estimated BPB are therefore reported simultaneously and kept in
separate namespaces.
