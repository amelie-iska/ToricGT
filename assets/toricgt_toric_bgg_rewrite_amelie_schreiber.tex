...
A graph-of-thought trajectory is a directed graph of reasoning vertices
\begin{equation}
  q_0\to q_1\to\cdots\to q_T,
\end{equation}
where each $q_t$ carries a hidden state, a behavior alcove, a finite complex $\Delta_t$, and a certificate signature $\Sigma_{\mathrm{TBGG}}(q_t)$.  The corridor objective is
\begin{equation}
\begin{aligned}
  \mathcal L_{\mathrm{BMP}}
  ={}&
  \mathcal L_{\mathrm{LM/BPB}}
  +\lambda_{\mathrm{corr}}\sum_{t}\dist(S_{\pi,C_t}(\xi_t),K_{\pi,C_t})^2
...
\begin{equation}
  R_{\mathrm{corr}}(\tau)=
  R_{\mathrm{task}}(\tau)
  \exp\left(-\gamma\sum_t\dist(S_{\pi,C_t}(\xi_t),K_{\pi,C_t})^2
  -\gamma_{\mathrm{alg}}\sum_t
  \bigl(\mathcal L_{\mathrm{SR}}^{\mathrm{beh}}+\mathcal L_{I_A}^{\mathrm{beh}}\bigr)\right).
\end{equation}
Thus the search policy prefers answers reached by helpful, algebraically coherent reasoning paths.
