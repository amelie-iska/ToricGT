...
structure training.  The present train split contains $917$ structure-association
records and no coordinate-bearing records.  Thus the run can train associations
between sequence, function, chemistry, and structure lookup hooks through the
graph LM and FoT/sidecar losses, but coordinate-flow matching, contact-map
binary cross-entropy, distogram cross-entropy, and frame/RMSD losses are
dormant until actual coordinate tensors are curated.  The implementation keeps
these losses coordinate-native: they operate on real batched coordinates and
masks, and they are not replaced by textual proxies.
