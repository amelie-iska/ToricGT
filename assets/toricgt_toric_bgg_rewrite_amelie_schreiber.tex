...
\ifPDFTeX
  ...
  \DeclareUnicodeCharacter{05DB}{k}
  \DeclareUnicodeCharacter{05EA}{t}
  \DeclareUnicodeCharacter{05D1}{b}
  \DeclareUnicodeCharacter{05BC}{}
  \DeclareUnicodeCharacter{05B8}{a}
  \DeclareUnicodeCharacter{05B7}{a}
\else
  \usepackage{fontspec}
\fi
...
\subsection{Embedding tropical ring attention into toric varieties}\label{subsec:tropical-ring-attention-toric-embedding}

The preceding toric diagnostics used Newton polytopes and normal fans.  The stronger and more reproducible construction is to embed each rationalized tropical attention head into a toric variety and run the geometric audits there.  This is the point at which TropicalGT and ToricGT become one project.  The statement is intentionally local and finite: a Transformer is not a toric variety in general, and a full neural checkpoint is not a scheme.  Instead, a checkpoint, layer, head, token window, and low-rank tropical probe determine a finite Laurent-polynomial or toric-ideal sidecar.  That sidecar lives in an algebraic torus, its tropicalization is controlled by initial degenerations, and a fan refining the relevant active or Groebner decomposition gives an ambient toric variety.  Toric tools are applied to this sidecar and to its orbit-stratified closure, not to the entire neural network.

Let $K$ be a nonarchimedean field with value group containing $\Q$; for implementation one may use Puiseux series with uniformizer $\tau$ and $\operatorname{val}(\tau)=1$.  Let
\begin{equation}
  N\simeq \Z^r,\qquad M=\operatorname{Hom}(N,\Z),
  \qquad T_N=\operatorname{Spec}K[M]
\end{equation}
be the cocharacter lattice, character lattice, and algebraic torus.  A tropical head or audit probe supplies, after rational quantization, characters
\begin{equation}
  a_1,\ldots,a_R\in M
\end{equation}
and affine candidate scores
\begin{equation}
  \ell_r(u)=\ip{a_r}{u}+b_r,\qquad u=P h\in N_\R,\quad b_r\in\Q,
\end{equation}
where $P$ is the learned or fixed projection from hidden state to the toric chart.  For a raw tropical attention head, the candidates are $S_{ij}+V_{jc}$; for the toric sidecar we use either an exact affine probe for those candidates or the certified local linearization on the bounded audit window.  Clearing denominators makes $a_r$ and $b_r$ integral after a common scaling.

\begin{definition}[Finite tropical attention sidecar]
Fix a checkpoint, layer, head, token window, projection $P$, and rationalization scale.  A finite tropical attention sidecar is the tuple
\begin{equation}
  \mathsf S_H=(K,N,M,A,b,P,\mathcal F_H,I_H,\Sigma,\eta),
\end{equation}
where $A=(a_1,\ldots,a_R)$ is the quantized exponent table, $b=(b_1,\ldots,b_R)$ is the quantized bias vector, $\mathcal F_H$ is the finite Laurent-polynomial family exported by the selected heads or output coordinates, $I_H=\langle\mathcal F_H\rangle$ when an ideal sidecar is used, $\Sigma$ is the chosen rational fan, and $\eta$ records the lattice basis, denominator-clearing scale, coefficient valuation convention, and the max-plus/min-initial sign convention.  The Laurent-polynomial sidecar audits active candidates and tie loci; the toric-ideal sidecar audits binomial relations, Groebner data, initial ideals, Hilbert/Betti data, and free resolutions.  Both sidecars are finite objects attached to this checkpoint/window only.
\end{definition}
...
\end{equation}
For $u\in N_\R$, use the initial form convention
\begin{equation}
  \operatorname{in}_{-u}(f)
  =
  \sum_{\substack{r:\\
    \operatorname{val}(\tau^{-b_r})+\ip{-u}{a_r}
    =\min_s(\operatorname{val}(\tau^{-b_s})+\ip{-u}{a_s})}}
    \overline{\tau^{-b_r}\chi^{a_r}}.
\end{equation}
Since
\begin{equation}
  \operatorname{val}(\tau^{-b_r})+\ip{-u}{a_r}
  ...
  -\ell_r(u),
\end{equation}
the minimum in the initial form is achieved exactly by the maximum in tropical attention.

\begin{proposition}[Tropical attention provenance as an initial degeneration]\label{prop:attention-initial-degeneration}
With the max-plus convention above,
\begin{equation}
  A(u)=\argmax_r \ell_r(u)
\end{equation}
is exactly the exponent support of $\operatorname{in}_{-u}(f)$.  The tropical hypersurface of $f$ is the tie locus where $\abs{A(u)}\ge2$, while the full attention provenance partition is the normal-cell decomposition recording $A(u)$ also on the unique-max chambers.
\end{proposition}

\begin{proof}
The initial form keeps precisely the monomials whose values
\begin{equation}
  -b_r-\ip{a_r}{u}
\end{equation}
are minimal.  Multiplying by $-1$ converts this condition to maximality of $\ip{a_r}{u}+b_r$.  Thus the retained monomials are exactly the active attention candidates.  The tropical hypersurface is the usual corner locus, where the initial form has at least two terms.  Tropical attention also records the open chambers where the initial form is monomial, because unique maximizers are reasoning decisions rather than failures of the tropical polynomial to be differentiable.
\end{proof}

For multiple output coordinates, heads, or graph tokens, we use a finite family
\begin{equation}
  \mathcal F_H=\{f_\alpha:\alpha\in\mathcal A_H\}\subset K[M]
\end{equation}
and the ideal $I_H=\langle \mathcal F_H\rangle$.  Its tropical variety is
\begin{equation}
  \operatorname{Trop}(Y_H)
  =
  \{u\in N_\R:\operatorname{in}_{-u}(I_H)\text{ contains no monomial}\},
  \qquad Y_H=V(I_H)\cap T_N,
\end{equation}
with the sign convention inherited from the max-plus attention head.  In practice, most attention states lie in chambers outside this tropical variety; the variety itself is the wall complex where candidate decisions tie.  The useful object for training is therefore the pair
\begin{equation}
  \bigl(\operatorname{Trop}(Y_H),\,\mathcal N_H\bigr),
\end{equation}
where $\mathcal N_H$ is the active normal or Groebner chamber complex.  The first gives balanced tropical cycles and Chow classes when the hypotheses hold; the second gives the decision strata used by the neural model.

Choose a rational fan $\Sigma$ in $N_\R$ refining all normal fans of the lifted Newton polytopes
\begin{equation}
  \Conv\{(a_r,b_r):r\le R\}
\end{equation}
and, when $I_H$ is used, refining the finite Groebner fan on the audited support.  The fan determines a toric variety $X_\Sigma$ with dense torus $T_N$.  On every cone $\sigma\in\Sigma$, the initial ideal $\operatorname{in}_{-u}(I_H)$ and every active set $A_\alpha(u)$ are constant for $u$ in the relative interior of $\sigma$, after sufficient refinement.

\begin{theorem}[Toric sidecar for tropical ring attention]\label{thm:tropical-attention-toric-sidecar}
Let $\Sigma$ be a rational fan refining the active normal complex of a finite tropical attention sidecar $\mathcal F_H$, and let $\overline Y_H$ be the closure of $Y_H=V(I_H)\cap T_N$ in $X_\Sigma$ when an ideal sidecar is present.  Then:
\begin{enumerate}[leftmargin=2em]
  \item every cone of $\Sigma$ labels a constant initial-form and attention-provenance regime;
  \item the torus orbit $O(\sigma)\subset X_\Sigma$ records the degeneration class indexed by $\sigma$;
  \item if $\abs{\Sigma}=\operatorname{Trop}(Y_H)$ and the multiplication map
  \begin{equation}
    T_N\times \overline Y_H\longrightarrow X_\Sigma
  \end{equation}
  is flat and surjective, then $\overline Y_H\subset X_\Sigma$ is a tropical compactification of $Y_H$ in the sense of Tevelev \cite{tevelev2007compactifications}; if the multiplication map is smooth and surjective, it is a schon tropical compactification;
  \item in the finite training regime, even when the global compactification hypotheses are not certified, $\Sigma$ still provides a reproducible toric audit object for active faces, orbit incidence, divisor bends, monomial initial ideals, and local free resolutions.
\end{enumerate}
\end{theorem}

\begin{proof}
The first claim is the defining property of a common refinement of the normal and Groebner decompositions.  Initial forms and active supports are locally constant on relative interiors of cones in such a refinement.  The second claim is the standard orbit-cone correspondence for toric varieties: cones $\sigma\in\Sigma$ correspond to torus orbits $O(\sigma)$, with orbit closures ordered by face relations.  The third claim is Tevelev's tropical compactification criterion \cite{tevelev2007compactifications}, stated here in the sign convention used by the attention sidecar.  The fourth claim follows because the data needed by the audits are finite combinatorial and commutative-algebraic data attached to $\Sigma$, $I_H$, and their initial ideals; those data remain well defined whether or not the closure is globally proper or smooth.
\end{proof}

This theorem is the mathematical guardrail for implementation.  ToricGT does not say that all hidden-state geometry is algebraic.  It says that when a head is used through max-plus candidates with rational exponents, the resulting finite tropical computation has a canonical toric envelope.  When the compactification hypotheses in item 3 are not certified, $X_\Sigma$ is called a \emph{finite toric audit envelope}.  The envelope still supplies active faces, orbit labels, one-dimensional-cone incidences, divisor-bend targets, monomial initial ideals, and local resolution data.  It does not by itself authorize claims about proper compactification, smooth boundary behavior, global Chow classes of $\overline Y_H$, or sheaf cohomology of the closure.

\paragraph{Divisors, Chow classes, and intersection metrics.}
The max-plus function
\begin{equation}
  \psi(u)=\max_r(\ip{a_r}{u}+b_r)
\end{equation}
is an integral piecewise-linear support function on a sufficiently refined fan after denominator clearing.  Hence it determines a torus-invariant Cartier divisor $D_\psi$ on $X_\Sigma$ when the usual Cartier compatibility condition holds.  If $\sigma$ and $\sigma'$ are adjacent maximal cones sharing a codimension-one cone $\tau$, and $m_\sigma,m_{\sigma'}\in M$ are the slopes of $\psi$ on them, the bend
\begin{equation}
  B_\psi(\tau)=\ip{m_\sigma-m_{\sigma'}}{u_{\sigma/\tau}}
\end{equation}
is the toric intersection of $D_\psi$ with the invariant curve associated to $\tau$, up to the standard primitive-normal convention.  Thus the empirical bend audit is not just a visualization: it is the finite neural analogue of a toric divisor intersection number.

When the tropical wall complex carries multiplicities $\omega(\sigma)$ on its maximal cones, balancedness is the condition that for every codimension-one cone $\tau$
\begin{equation}
  \sum_{\sigma\supset\tau}\omega(\sigma)u_{\sigma/\tau}=0
  \quad\text{in }N/N_\tau.
\end{equation}
The training-side residual is
\begin{equation}
  \mathcal L_{\mathrm{bal}}
  =
  \sum_{\tau}
  \left\|
    \sum_{\sigma\supset\tau}\widehat\omega(\sigma)u_{\sigma/\tau}
  \right\|_2^2.
\end{equation}
By Fulton-Sturmfels and the tropical intersection viewpoint, a balanced codimension-$c$ fan defines a Minkowski weight, hence a class in $A^c(X_\Sigma)$ \cite{fultonsturmfels1994intersection,katz2009tropicalintersection}.  The corresponding Chow-class metric compares predicted and certified Minkowski weights:
\begin{equation}
  \mathcal L_{\mathrm{Chow}}
  =
  \sum_{\sigma\in\Sigma(c)}
  \abs{\widehat\omega(\sigma)-\omega^\star(\sigma)}^2.
\end{equation}
Maclagan and Rincon's theorem that varieties of tropical ideals are balanced justifies this class when the sidecar is a tropical ideal or the tropicalization of a classical ideal \cite{maclaganrincon2018tropicalideals,maclaganrincon2020balanced}.

\paragraph{Toric ideals and Miller-Sturmfels staircases.}
Let $A=[a_1,\ldots,a_R]$ be the exponent matrix.  The monomial map
\begin{equation}
...
\end{equation}
This is the exact commutative-algebra object behind the binomial consistency loss.  For two-parameter local persistence or two-coordinate attention slices, a monomial initial ideal
\begin{equation}
  J=\langle x^{a_1}y^{b_1},\ldots,x^{a_s}y^{b_s}\rangle,
  \qquad a_1>\cdots>a_s,\quad b_1<\cdots<b_s,
\end{equation}
is represented by the Miller-Sturmfels staircase: lattice points outside $J$ form the monomial basis of $K[x,y]/J$, and the boundary staircase is the interface between retained and killed monomials.  The minimal first syzygies in the generic two-variable case come from adjacent generators rather than all pairs:
\begin{equation}
  0\longrightarrow
  \bigoplus_{i=1}^{s-1}K[x,y](-\operatorname{lcm}(m_i,m_{i+1}))
  \longrightarrow
  \bigoplus_{i=1}^{s}K[x,y](-m_i)
  \longrightarrow
  J
  \longrightarrow 0.
\end{equation}
This is the correct visualization for grid modules in ToricGT: not a Pareto frontier, but stacked staircase layers, with small height offsets for reasoning level, filtration radius, or internal degree.  The Hilbert numerator, Betti table, and staircase area become exact sidecar metrics for two-parameter persistence modules and local initial ideals \cite{millersturmfels2005cca}.

\paragraph{Vector bundles, sheaves, and Klyachko losses.}
Toric vector bundles add a higher-rank version of the same control mechanism.  A toric vector bundle on $X_\Sigma$ is classified by a finite-dimensional vector space $E$ and decreasing filtrations
\begin{equation}
  \cdots\supseteq E^\rho(i)\supseteq E^\rho(i+1)\supseteq\cdots
\end{equation}
for every one-dimensional cone $\rho\in\Sigma(1)$, satisfying Klyachko compatibility: for every cone $\sigma\in\Sigma$ there is a decomposition
\begin{equation}
  E=\bigoplus_{m\in M}E_m^\sigma
\end{equation}
such that, for every one-dimensional cone $\rho\preceq\sigma$ with primitive generator $u_\rho$,
\begin{equation}
  E^\rho(i)=
  \bigoplus_{\ip{m}{u_\rho}\ge i}E_m^\sigma.
\end{equation}
ToricGT can treat hidden channels inside a head as the fiber $E$ and use attention gates or GraphCG axes as learned filtrations.  The Klyachko residual is
\begin{equation}
  \mathcal L_{\mathrm{Kly}}
  =
  \sum_{\sigma}\sum_{\rho\preceq\sigma}\sum_i
  \left\|
    \Pi^\rho_i
    -
    \sum_{\ip{m}{u_\rho}\ge i}\Pi_m^\sigma
  \right\|_F^2
  +
  \lambda_{\mathrm{flag}}
  \sum_{\rho,i}\norm{\Pi^\rho_{i+1}\Pi^\rho_i-\Pi^\rho_{i+1}}_F^2,
\end{equation}
where $\Pi^\rho_i$ and $\Pi_m^\sigma$ are predicted projection matrices.  The first term checks compatibility with local weight decompositions; the second checks that the filtration is nested.  Recent tropical vector-bundle work gives two additional views: tropical toric vector bundles can be encoded by valuated matroids and flats over tropical toric bases, and toric vector bundles can be described as tropical or valuation-valued objects whose positivity becomes convexity of a piecewise-linear map \cite{khanmaclagan2024tropicalbundles,junminchevatolliver2023tropicalschemes,kavehmanon2023toricbundles}.  These are natural late-phase losses for representation organization, not early BPB objectives.

For sheaves and Cox coordinates, the fan gives the Cox ring
\begin{equation}
...
\begin{equation}
  ...
  \right\rangle.
\end{equation}
Equivariant sheaves become graded $S_\Sigma$-modules modulo the irrelevant locus.  This is the natural bridge from tropical attention sidecars to BGG/Tate supervision: Macaulay2 can compute free resolutions, Betti tables, local cohomology, Fitting ideals, and Buchsbaum-Eisenbud rank/minor checks for the finite graded modules emitted by a checkpoint.

\paragraph{CAS-reproducible audit contract.}
The exact audit path is:
\begin{enumerate}[leftmargin=2em]
  \item export a sidecar JSON containing the rational exponent table $A$, biases $b$, lattice basis, denominator-clearing scale, coefficient valuation convention, max-plus/min-initial sign convention, head/layer ids, active sets, projected hidden coordinates, and any ideal generators;
  \item use Sage, polymake, or Normaliz-compatible routines to build Newton polytopes, normal fans, orbit incidence, divisor support functions, and one-dimensional-cone divisor data;
  \item use Macaulay2 with \texttt{Tropical}, Gfan, and Polymake integration to compute tropical varieties, cones, multiplicities, balancing, initial ideals, and min/max-convention checks \cite{m2tropical2019};
  \item use Macaulay2 or Sage for toric ideals, Groebner bases, Hilbert series, Betti tables, free resolutions, Fitting ideals, and monomial-staircase diagrams;
  \item use toric-vector-bundle or explicit Klyachko sidecars to compute filtration compatibility, Chern-class proxies, parliament-of-polytopes summaries, and convexity/global-generation checks where available;
  \item cache every certified object by a hash of $(A,b,\Sigma,I_H,\text{conventions})$ and send only compact tensors back to the GPU.
\end{enumerate}
There should be no silent proxy for these exact audits.  If a CAS computation is unavailable, too large, or fails its convention check, the metric is marked unavailable for that batch and excluded from the training loss.  Differentiable neural losses are allowed only after the exact small-window CAS audit has validated their target on representative examples.

\begin{center}
\scriptsize
\begin{tabular}{@{}p{0.17\linewidth}p{0.29\linewidth}p{0.22\linewidth}p{0.17\linewidth}@{}}
\toprule
Object & Exact sidecar & Differentiable surrogate & Failure mode \\
\midrule
Initial forms & rational $\operatorname{in}_{-u}(\mathcal F_H)$ or $\operatorname{in}_{-u}(I_H)$ & active-set cross entropy or support distance & unavailable if sign or valuation metadata disagree \\
Fan and bends & Sage/Normaliz fan data, primitive normals, Cartier compatibility & fan-margin and bend-matching losses & audit envelope only if refinement or Cartier data are incomplete \\
Cycles and Chow data & balanced multiplicities or certified Minkowski weights & balance and Chow residuals & visualization only if balancing or tropical-ideal hypotheses fail \\
Toric ideals and resolutions & Macaulay2 toric ideals, Groebner bases, Hilbert series, Betti tables, free resolutions, Fitting ideals & binomial, Betti-vector, differential, and rank/minor residuals & skip loss if the bounded window exceeds CAS limits \\
Bundles and sheaves & Klyachko filtrations, Cox modules, local cohomology, Chern or parliament summaries when available & nestedness, cone-compatibility, gluing, and convexity proxies & late-phase only; disabled before exact toy-window certification \\
\bottomrule
\end{tabular}
\end{center}

The resulting late-phase toric objective refines the earlier compact loss:
\begin{equation}
\begin{split}
  \mathcal L_{\mathrm{toric\text{-}embed}}
  ={}&
  \lambda_{\mathrm{init}}\mathcal L_{\mathrm{initial}}
  +\lambda_{\mathrm{fan}}\mathcal L_{\mathrm{fan}}
  +\lambda_{\mathrm{bend}}\mathcal L_{\mathrm{bend}}
  +\lambda_{\mathrm{bal}}\mathcal L_{\mathrm{bal}}
  +\lambda_{\mathrm{Chow}}\mathcal L_{\mathrm{Chow}}\\
  &+
  \lambda_{\mathrm{binom}}\mathcal L_{\mathrm{binom}}
  +\lambda_{\mathrm{res}}\mathcal L_{\mathrm{resolution}}
  +\lambda_{\mathrm{Kly}}\mathcal L_{\mathrm{Kly}}
  +\lambda_{\mathrm{sheaf}}\mathcal L_{\mathrm{sheaf}}.
\end{split}
\end{equation}
Here $\mathcal L_{\mathrm{initial}}$ aligns predicted active supports with CAS-certified initial forms, $\mathcal L_{\mathrm{resolution}}$ compares predicted and certified Betti or differential data, and $\mathcal L_{\mathrm{sheaf}}$ covers Cox-module cohomology or Fitting-rank residuals.  These weights remain zero through early language-model training and are activated only in the audit-gated late phases described in \cref{subsec:checkpoint-triage}.  The BPB rule remains unchanged: a toric sidecar earns deployable bytes only if it improves score-after-export or a predeclared reasoning metric without a BPB regression.
...
The analysis suite implements this contract with interactive 3D diagnostics.  For each sampled graph-of-thought branch it plots focused hidden-state coordinates, local Vietoris--Rips 1/2-complexes, toric active-face colors, chamber-crossing markers, empirical normal-fan one-dimensional cones, convex chamber-polytopes for occupied active faces, fitted wall sheets at observed tropical/toric crossings, and a small Newton-polytope shadow built from the pseudo-exponents used by the empirical fan audit.  The energy-landscape companion lifts local NLL to the vertical axis and overlays the same chamber and wall geometry.  Sparse/default/dense controls adjust the local simplicial radius; Plotly legend toggles isolate branches, complexes, chambers, walls, and polytopes.
...
\paragraph{Metric taxonomy and active-tokenizer conversion.}
The implementation keeps tokenizer-specific FineWeb quantities separate.  The official
...
its endpoint ranks.  Under these hypotheses the active-tokenizer graph-node
loss is a score-before-update next-token loss, and the byte denominator is the
sum of target token byte increments, so the restricted FineWeb metric can be
...
\paragraph{ConvexTok tokenizer graph.}
The current BPB campaign adds ConvexTok-2048 Det as an active-tokenizer option.
ConvexTok constructs a byte-boundary DAG: vertices are byte offsets, free
one-byte edges preserve feasibility, and priced substring edges carry
vocabulary colours.  A sparse LP relaxation chooses colour masses on a
tokenizer-training sample, and deterministic or bias-weighted rounding selects
the deployable vocabulary.  With the vocabulary fixed, encoding is exact
min-plus dynamic programming on this DAG.  The path length, LP lower bound,
regret ratio, active-edge entropy, and top-two path margin are logged as
tokenizer-level tropical metrics.  The selected token incidence vectors, LP
scores, rounded ranks, byte lengths, and priced/free flags define a finite
toric vocabulary-polytope shadow; token-path changes are treated as normal-fan
wall crossings.  In the OAI baseline, \texttt{CONVEXTOK\_DAG\_FEATURES=1}
feeds those features into the first-class TokenGT structural embedding, and
\texttt{CONVEXTOK\_TORIC\_REG\_WEIGHT} applies a small training-only embedding
regularizer.  The scored object remains the flattened active-tokenizer stream
with exact byte accounting, so the tokenizer geometry cannot replace the BPB
contract.
...
\begin{enumerate}[leftmargin=2em]
  ...
  \item BPB-safe first-class TokenGT graphification for OAI FineWeb: every scored active-tokenizer token, whether SP1024 or ConvexTok, is treated as a graph node with deterministic low-rank node identifiers, endpoint-pair features, local one-dimensional-edge features, toric phase features, and virtual causal edge-token states folded back into node states;
  \item an OAI-FineWeb-only graph-output flattening and score-correction adapter, so graph-in/graph-out hidden computation is trained while official BPB is still computed on the terse sequential target;
...
The active OAI-baseline adaptation uses the same principle with either the
SP1024 baseline tokenizer, the ConvexTok-2048 Det tokenizer, or the current
fresh ConvexTok-8192 biomedical tokenizer.  The ConvexTok-8192 restart is not
BPE and does not reuse the ConvexTok-2048 vocabulary.  It keeps byte fallback
and a biomedical/control reserve for UniProt-style annotations, sequence
motifs, PDB/AFDB/ESMFold structure tags, molecule strings, atomistic trajectory
metadata, toric/tropical diagnostics, persistence signatures, BGG certificates,
and Forest-of-Thought control tokens.  The current mu-transfer scale-up uses
$V=8192$, $d=1536$, $H=12$, six KV heads, $L=9$, tied embeddings, RoPE with
\texttt{ROPE\_BASE}=10000, sequence length $1024$, and about $1.7\times10^8$
parameters.  First-class FineWeb TokenGT identifiers and graph-output
flattening remain enabled; the official BPB score is still computed on the
flattened active-tokenizer sequence.
The relevant exposed knobs are
\texttt{TOKENGT\_IDENTIFIER\_WEIGHT},
\texttt{TOKENGT\_ENDPOINT\_WEIGHT},
\texttt{TOKENGT\_EDGE\_TOKEN\_WEIGHT},
\texttt{GRAPH\_OUTPUT\_EDGE\_TOKEN\_WEIGHT}, and
\texttt{GRAPH\_OUTPUT\_SCORE\_CORRECTION\_WEIGHT}.  They are deliberately
logged separately because BPB evidence for one graph component should not be
treated as evidence against all graphification.
...
The gate writes an audit record containing train BPB, validation BPB when
available, answer-span BPB, branch-search lift, graph-answer accuracy,
graph-LM BPB, graphification and flattening weights, resolution-consistency
score, standard-filtration leakage, Koszul-linearity residual, Gale-dual
consistency, BGG trajectory retrieval at $K$, GraphCG covariance, tropical
active-face margins, and artifact bytes after round-trip export.  A run may
continue with a larger Toric BGG weight only if the record shows no
score-first violation and no statistically meaningful BPB regression relative
to a matched seed or matched checkpoint window.
...
The current campaign cadence is more aggressive than the older 5K-step loop:
each attempt runs to a 1.5K-step gate with target BPB $<1.19$.  If the target
is missed, the supervisor runs the full analysis suite, reviews all W\&B
metrics including the \texttt{toricgt\_sidecar} family, writes a timestamped
report, selects adaptive environment overrides, and restarts from step $0$.
The graph-structure controller keeps FineWeb graphification and OAI-only
flattening on, but sweeps node-identifier, endpoint, virtual-edge, and
score-correction weights independently of GraphCG, analogy, BGG, Koszul,
toric, vector-bundle/sheaf, and combinatorial-commutative-algebra losses.
...
\subsection{ToricBLM biological Forest-of-Thought records}

The ToricBLM preparation path adds a biological graph/FoT slice derived from
local mirrors of UniProt function text, UniRef50 sequence clusters, DNA coding
regions, Rfam sequence records, RNAcentral records, and PubChem SELFIES strings.
The raw mirrors remain symlinked into the ToricGT repository rather than copied,
so the training code can use them without duplicating tens of gigabytes of
source data.  Each curated row contains the ordinary text view plus
\texttt{graph\_json}, \texttt{thought\_forest\_json},
\texttt{convextok\_dag\_json}, \texttt{training\_views\_json}, and
\texttt{enrichment\_status\_json}.  The forest view is source-grounded: sequence,
annotation, structure-lookup, design-condition, and chemistry trees expand from
fields actually present in the row, while absent UniProtKB, PDB, AFDB, site,
kinetic, pathway, or coordinate information is routed through explicit
self-correction edges instead of being synthesized.

The current balanced slice has $3072$ records, $512$ from each of the six local
source families.  A leakage-aware split assigns entire clusters to one split,
yielding $2788$ train, $156$ validation, and $128$ test records.  The cluster
signature combines sequence sketches, function labels and text shingles,
structure lookup hooks, source-graph histograms, and FoT graph topology.  This
is a finite leakage guard for the data actually present.  It is not yet a claim
of full structural decontamination: when coordinate-bearing PDB, AFDB, UMA
MLIP, BioEmu, BioKinema, ConfRover, or other trajectory records are added, the
splitter should be upgraded with Foldseek-style structural clustering or
ProTrek-style trimodal sequence/structure/function contrastive embeddings.

The ConvexTok audit on this slice compares the fresh ConvexTok-8192
biomedical tokenizer with the older ConvexTok-2048 tokenizer.  ConvexTok-8192
reduces audited graph/FoT token count by $7.21\%$ and increases priced-token
coverage, but raw sequence fields still use byte fallback heavily.  The
pragmatic decision is therefore to train the immediate full run with
ConvexTok-8192 and schedule a later vocabulary-extension pass over amino-acid
motifs, nucleotide motifs, GO/EC syntax, residue-pair fields, atom and chain
labels, coordinate metadata, and dynamics tags.

The same audit distinguishes structure association from coordinate-native
structure training.  The balanced graph/FoT train split contains $917$
structure-association records and no coordinate tensors, so it trains
associations between sequence, function, chemistry, and structure lookup hooks
through the graph LM and FoT/sidecar losses.  Coordinate-native supervision is
provided by a separate AFDB v6 shard with $256$ AlphaFold mmCIF-derived
records, split into $239$ train, $7$ validation, and $10$ test examples.  These
rows carry real CA/backbone coordinate tensors, coordinate masks, pLDDT, and
graph/FoT structure context.  The adapted OAI baseline trains rectified-flow
velocity matching, contact-map binary cross-entropy, distogram cross-entropy,
and centered RMSD/frame diagnostics on those real coordinate batches.  Rows
without usable AFDB coordinates are skipped rather than filled with proxy
targets, so the structure losses remain coordinate-native.
...
\begin{enumerate}[leftmargin=2em]
  ...
  \item \texttt{embedding\_forest\_of\_thought.py}: training-only embedding-space Forest-of-Thought head, sparse activation, UCB expansion pressure, hidden self-correction, consensus, and trajectory/subtrajectory balance metrics.
...
\begin{enumerate}[leftmargin=2em]
  ...
  \item \textbf{Toric graph-of-thought tasks}: serialize the above objects as typed graphs with cone, one-dimensional-cone, face, divisor, monomial, and wall nodes, so the same graph-token machinery handles polyhedral geometry and ordinary reasoning records.
\end{enumerate}
These tasks should be held out by fan combinatorial type, number of one-dimensional cones, Newton-polytope dimension, relation-rank, and wall-equivalence class.  Random example-level splits are especially weak here because adjacent points in a normal fan can share almost all combinatorial data.
...
\subsection{Embedding-space Forest-of-Thought}

The current ToricGT implementation adds a second inference-scaling and training-time search layer inspired by Tree-of-Thought, Graph-of-Thought, and Forest-of-Thought methods \cite{yao2023tree,besta2023graph,bi2024forest}.  The original FoT proposal runs several reasoning trees, activates only relevant branches, revisits flawed states through self-correction, and selects final answers by consensus.  ToricGT imports those operations into hidden space rather than prompt space.  This distinction is important for the Parameter-Golf track: the official scored object remains a byte-level prequential distribution, but the unscored auxiliary heads can train hidden states to support branching, correction, and consensus.

\begin{definition}[Embedding Forest-of-Thought state]
For a prefix-visible hidden table $H=(h_1,\ldots,h_L)$, an embedding-space FoT state is
\begin{equation}
  \mathfrak F(H)=
  \left(
    \{T_\alpha\}_{\alpha=1}^{A},
    a_{i\alpha},
    v_{i\alpha},
    u_{i\alpha},
    c_i,
  ...
  \right),
\end{equation}
where $T_\alpha$ is a latent thought tree, $a_{i\alpha}$ is the activation mass assigning token or graph position $i$ to tree $\alpha$, $v_{i\alpha}$ is a value estimate, $u_{i\alpha}$ is a UCB-style expansion score, $c_i$ is a consensus logit over a compact bucketed leaf space, $\delta_i$ is a correction vector, and $z$ is the learned log partition used by trajectory balance.  The node set of $T_\alpha$ is not serialized; it is a structured view of hidden positions, graph reasoning levels, and optional memory items.
\end{definition}

The tree activations are intentionally sparse.  With temperature $\tau_F$ and learned tree keys $k_\alpha$, the implementation uses
\begin{equation}
  a_{i\alpha}=
  \operatorname{softmax}_\alpha
  \left(\frac{\ip{W_a h_i}{k_\alpha}}{\tau_F}\right),
  \qquad
  A_i=\operatorname{TopK}_\alpha a_{i\alpha}.
\end{equation}
Sparse activation is measured by entropy and active-tree count.  A collapse to one tree too early indicates that the FoT head has become a single-path regularizer; diffuse activation across all trees indicates that it is wasting auxiliary compute.

The reward is a training-only scalar that combines BPB pressure with advanced-structure agreement:
\begin{equation}
  R_i=
  \sigma\left(
    \rho_0-\operatorname{NLL}_i
    +\eta_G R_i^{\mathrm{GFN}}
    +\eta_T R_i^{\mathrm{toric}}
    +\eta_{\mathrm{top}}R_i^{\mathrm{top}}
    +\eta_{\mathrm{mem}}R_i^{\mathrm{mem}}
  \right).
\end{equation}
When validation BPB is unavailable during an early gate, the controller falls back to train BPB and local NLL trends.  The advanced rewards may shape the forest, but they are routed through the same auxiliary-gradient controller as GraphCG, memory, BGG, Koszul, and toric losses; if they conflict with the byte objective, their gradients are damped or projected.

\begin{definition}[Embedding FoT loss]
The implemented FoT objective is
\begin{equation}
\begin{split}
  \mathcal L_{\mathrm{FoT}}={}&
  \lambda_{\mathrm{sp}}\mathcal L_{\mathrm{sparse}}
  +\lambda_{\mathrm{ucb}}\mathcal L_{\mathrm{UCB}}
  +\lambda_{\mathrm{corr}}\mathcal L_{\mathrm{corr}}
  +\lambda_{\mathrm{cons}}\mathcal L_{\mathrm{cons}}\\
  &+\lambda_{\mathrm{TB}}\mathcal L_{\mathrm{TB}}
  +\lambda_{\mathrm{SubTB}}\mathcal L_{\mathrm{SubTB}}
  +\lambda_{\mathrm{cmp}}\mathcal L_{\mathrm{complexity}} .
\end{split}
\end{equation}
The sparse term controls activation entropy; the UCB term encourages expansion toward branches with high value and uncertainty; the correction term aligns $\delta_i$ with lower-NLL or higher-reward successor directions; the consensus term asks high-reward leaves to agree in a compact decision bucket; trajectory and subtrajectory balance make the forest a reward-proportional sampler; and the complexity term prevents unbounded tree growth.
\end{definition}

The UCB pressure uses the standard form
\begin{equation}
  u_{i\alpha}=v_{i\alpha}+
  \beta_{\mathrm{ucb}}
  \sqrt{
    \frac{\log(1+\sum_\gamma n_{i\gamma})}
         {1+n_{i\alpha}}
  },
\end{equation}
where $n_{i\alpha}$ is a differentiable visit proxy induced by activation mass.  The correction term uses the local hidden displacement:
\begin{equation}
  \mathcal L_{\mathrm{corr}}
  =
  \Eop_i
  \left[
    w_i\left(1-\cos(\delta_i,h_{i+1}-h_i)\right)
  \right],
  \qquad
  w_i=\max(0,\operatorname{NLL}_i-\operatorname{NLL}_{i+1}) .
\end{equation}
This is a hidden-state analogue of FoT self-correction: when a successor improves the score, the correction vector should point toward that improvement.

For a forest trajectory $\tau=(s_0,\ldots,s_T)$, the trajectory-balance term is
\begin{equation}
  \mathcal L_{\mathrm{TB}}(\tau)=
  \left(
    z+\sum_{t=0}^{T-1}\log P_F(s_{t+1}\mid s_t)
    -\log R(s_T)
    -\sum_{t=0}^{T-1}\log P_B(s_t\mid s_{t+1})
  \right)^2 .
\end{equation}
Subtrajectory balance applies the same relation to windows inside $\tau$ \cite{madan2023subtb}.  In the current OAI baseline adaptation, $P_F$ and $P_B$ are compact learned projections from hidden states and forest activations; they do not add tokens to the scored byte stream.

\begin{proposition}[BPB-safe FoT auxiliary]
Assume the FoT head is excluded from the serialized Parameter-Golf artifact and its gradients are routed through a controller that caps the auxiliary contribution by a fixed ratio of the current cross-entropy gradient norm.  Then enabling $\mathcal L_{\mathrm{FoT}}$ cannot by itself invalidate the byte-level BPB scoring protocol.  It can only change the learned base-model parameters through training-time gradients, which are evaluated by the ordinary score-before-update BPB audit.
\end{proposition}

\begin{proof}
The official scorer sees only the exported byte model and its normalized next-byte distributions.  The FoT head does not introduce future byte values into the context, does not add scored tokens, and is omitted from export unless separately byte-accounted.  During training it contributes an auxiliary gradient bounded by the controller.  Therefore any effect on BPB must be mediated through ordinary parameter updates and is measured by the same validation protocol as every other training loss.
\end{proof}

Operationally, FoT metrics are reported under \texttt{oai\_fot/*} and then organized in the W\&B namespace \texttt{05\_forest\_of\_thought/*}.  The periodic review agent reads FoT loss, reward, activation entropy, active-tree count, top-$K$ tree mass, tree diversity, consensus entropy, consensus margin, tree agreement, correction cosine, correction BPB-proxy lift, and trajectory-balance residual separately rather than lumping them into a generic ``advanced loss'' bin.  At each gate the adaptive controller may increase tree count, lower activation temperature, raise correction weight, lower UCB pressure, or reduce FoT loss weight depending on how these quantities correlate with train BPB, validation BPB, graph-LM loss, and sidecar metrics.
...
\bibitem{maclagan2012intro}
Diane Maclagan.
\newblock Introduction to tropical algebraic geometry.
\newblock arXiv:1207.1925, 2012.
...
\bibitem{schock2024quasilinear}
Nolan Schock.
\newblock Quasilinear tropical compactifications.
\newblock arXiv:2112.02062, 2024.
...
\bibitem{millersturmfels2005cca}
...
\newblock \emph{NeurIPS}, 2023. arXiv:2305.10601.

\bibitem{besta2023graph}
Maciej Besta, Nils Blach, Ales Kubicek, Robert Gerstenberger, Micha\l{} Podstawski, Lukas Gianinazzi, Joanna Gajda, Tomasz Lehmann, Hubert Niewiadomski, Piotr Nyczyk, and Torsten Hoefler.
\newblock Graph of Thoughts: Solving elaborate problems with large language models.
\newblock arXiv:2308.09687, 2023.

\bibitem{bi2024forest}
Zhenni Bi, Kai Han, Chuanjian Liu, Yehui Tang, and Yunhe Wang.
\newblock Forest-of-Thought: Scaling test-time compute for enhancing LLM reasoning.
\newblock arXiv:2412.09078, 2024.
