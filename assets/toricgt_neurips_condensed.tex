...
\]
and also records Taylor-resolution ranks and boundary-term counts for $I_\Delta$.  The train-time version evaluates the same multigraded polynomial ring on soft chamber variables $x_i(H)=n^{-1}\sum_t p_i(h_t)$, so monomial losses such as $\prod_{i\in F}x_i(H)$, Taylor lcm-syzygy masses, and Hilbert/Betti pressure are differentiable polynomial maps.  Exact symbolic resolutions supply targets and audit coefficients; the optimizer sees those exact multigraded polynomials evaluated on differentiable chamber coordinates.
