...
The next inference-scaling layer is anticipative trajectory memory.  Completed graph-of-thought paths are stored as compact keys
\begin{equation}
  k(\tau)=\operatorname{norm}\big[\bar h,\;h_T-h_1,\;v(\tau),\;\kappa(\tau),\;\rho_{\mathrm{VR}}(\tau),\;\phi_\theta(\tau),\;\phi_\beta(\tau),\;q(\tau)\big],
\end{equation}
where \(v,\kappa\) summarize speed and curvature, \(\rho_{\mathrm{VR}}\) is a local simplicial density/radius statistic, \(\phi_\theta,\phi_\beta\) are noncommutative toric phase-shadow moments, and \(q\) is a trajectory-quality proxy.  A small retrieval head predicts which memory item helps the current state:
\begin{equation}
...
\begin{equation}
  \tilde r(s,i)=\lambda_G\ip{g_s}{g_i}+\lambda_\Theta\ip{\phi_s}{\phi_i}-\lambda_T d_{\mathrm{top}}(s,i)+q_i,
\end{equation}
...
\end{equation}
where the terms measure resolution consistency, standard-filtration leakage, Koszul-linearity residual, Gale-dual signature consistency, and BGG-signature smoothness.  These probes are excluded from the Parameter-Golf artifact with the other training-only heads.  In the current recovery config they are instantiated for metrics but have zero loss weight until late GFlowNet/GraphCG/topology phases; after warmup they enter at the same tiny scale as toric/Koszul diagnostics and remain subject to the BPB gate.

The live \texttt{oai} training run currently uses a BPB-recovery phase rather than the full auxiliary-geometry objective.  A revealed-context recovery run from step $1500$ showed a local likelihood valley near steps $1545$--$1550$ and then rebounded by step $1700$.  The restart therefore returns to \texttt{random\_order\_step\_00001500.pt}, resets Adam moments, fixes the guard parser so explicit zero scalars remain zero, keeps medium/hard graph rows off, sets shock-update scale to $0$, disables all high-curvature geometry losses through the first repair gates, and pauses first at step $1525$ for fixed BPB, reasoning, topology, toric, and Toric BGG analysis.  If validation BPB does not improve by the next planned gates at $1550$, $1575$, and $1600$, the policy is to stop or edit rather than continue from a shallow train-only valley.  The heavier toric, Koszul, Toric BGG, flow, memory, and QAT terms stay diagnostic until the analysis suite and Codex handoff choose a later-phase promotion.

\begin{figure}[t]
  \centering
  \includegraphics[width=0.86\linewidth]{fig_oai_step2250_reasoning_k_bpb.png}
...
\begin{figure}[t]
  ...
  \caption{Irrational toric phase projection of a reasoning trajectory.  The torus path shows pseudo-periodic phase coverage; faint local edges are Vietoris--Rips 1-simplices; magenta arrows represent window-to-window analogical maps.}
...
\end{equation}
Here $A$ is a small semigroup exponent table, $\Delta$ is a chamber complex, $\mathcal R$ is a verified low-degree toric-binomial relation set, $(C_\bullet,\partial_\bullet)$ is a finite chain complex, $T_i$ are local Koszul operators, and $\mathcal F_\ell$ are filtered flag complexes.  Exact sidecar checks verify $Au=Av$ for $x^u-x^v\in\mathcal R$, $\partial_{q-1}\partial_q=0$, $\mathcal F_\ell\subseteq\mathcal F_{\ell+1}$, and $T_iT_j=T_jT_i$ when Koszul exactness is claimed.  The train-time scalar is a bounded surrogate,
\begin{equation}
...
\end{equation}
where $\mathcal L_{\mathrm{binom}}$ is a toric-ideal residual and $\mathcal L_{\mathrm{SR}}$ is a Stanley--Reisner nonface mass.  A metric moves from diagnostic to loss only when the exact finite audit agrees with the differentiable surrogate and matched validation shows no BPB regression.  This is the operative meaning of ``toric'' in the compact training loop: finite algebraic certificates first, small gated gradients second.
