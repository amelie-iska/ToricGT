...
The live \texttt{oai} training run currently uses a BPB-recovery phase rather than the full auxiliary-geometry objective.  The latest step-$2250$ gate found that train BPB and loss were descending but flattening: median train BPB fell from $3.8067$ to $3.5449$, with recent slope about $-1.92$ BPB per $1000$ optimizer steps, while GraphCG covariance and analogical-map diagnostics drifted under a diagnostic-only GraphCG phase.  The resumed run therefore activates a small GraphCG basis term, $\lambda_{\mathrm{GraphCG}}=6\times10^{-5}$, a small analogy-lattice term, $10^{-5}$, a small GFlowNet term, $\lambda_{\mathrm{GFN}}=2.5\times10^{-4}$ with entropy weight $5\times10^{-5}$, contrastive weight $8\times10^{-5}$, LR multiplier $0.16$, and clip norm $0.42$, while keeping toric, Koszul, flow, and QAT losses at zero until the next gate.  The completed step-$2250$ geometry gate evaluated three held-out validation records and nine graph-of-thought branches: mean branch BPB was $4.804$, best branch BPB was $3.476$, mean answer-span BPB was $4.776$, best answer-span BPB was $3.476$, MST efficiency was $0.667$, path smoothness was $0.0480$, directed asymmetry was $0.345$, HDBSCAN stability was $0.931$, Slepian concentration was $1.000$, and Slepian leakage was $0.000$.  The same audit found weak tropical/toric margins, so heavy toric geometry remains diagnostic while GraphCG improves the coordinate chart for later chamber losses and GFlowNet begins learning from the observed branch gap.

\begin{figure}[t]
  \centering
  \includegraphics[width=0.86\linewidth]{fig_oai_step2250_reasoning_k_bpb.png}
  \caption{Step-$2250$ \texttt{oai} recovery diagnostic.  Branches are embedded in the reasoning/K$(x\mid\mathrm{helpers})$/BPB simplex from model-computed statistics.  The early checkpoint still favors the low-BPB corner over longer reasoning budgets, so GraphCG and a small GFlowNet objective are activated while heavier toric/Koszul geometry stays gated.}
