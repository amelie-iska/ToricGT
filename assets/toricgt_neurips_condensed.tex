...
The live \texttt{oai} training run currently uses a BPB-recovery phase rather than the full auxiliary-geometry objective.  After checkpoint \texttt{random\_order\_step\_00036750.pt}, local BPB curvature flattened, so GFlowNet, toric, Koszul, flow, and QAT losses were set to zero while small contrastive, multi-token, and GraphCG pressures were retained.  The completed step-$37250$ analysis gate evaluated four held-out validation records and $24$ graph-of-thought branches: mean branch BPB was $4.758$, best branch BPB was $4.039$, mean answer-span BPB was $4.644$, best answer-span BPB was $3.630$, MST efficiency was $0.507$, path smoothness was $0.0895$, directed asymmetry was $0.401$, HDBSCAN stability was $0.943$, Slepian concentration was $1.000$, and Slepian leakage was $0.000$.  The same audit found weak tropical active-face margins, so geometry remains diagnostic until byte-likelihood descent recovers.  The watcher now resolves W\&B histories by either run id or human-readable run name, preventing analysis pauses from becoming terminal due to path mismatch.

\begin{figure}[t]
  \centering
  \includegraphics[width=0.86\linewidth]{fig_oai_step37250_reasoning_k_bpb.png}
  \caption{Step-$37250$ \texttt{oai} recovery diagnostic.  Branches are embedded in the reasoning/K$(x\mid\mathrm{helpers})$/BPB simplex from model-computed statistics.  The plot is an audit snapshot used to decide whether auxiliary geometric losses should stay active or remain disabled.}
  \label{fig:oai-step37250-condensed}
\end{figure}
