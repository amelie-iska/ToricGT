...
\begin{abstract}
We present ToricGT, a compact graph-to-graph Transformer family for algebraic, mathematical, morphological, and parameter-constrained language-model reasoning.  The research model tokenizes nodes and edges as a shared sequence, uses exact blockwise ring evaluation for tropical max-plus attention, adds finite noncommutative-torus phase channels, and uses permutation-equivariant Soft-MoE routing in upper graph blocks.  The paper contributes a finite theory and an implementation-facing training contract.  We prove graph-token equivariance, exactness of tropical ring attention, stability of active faces under perturbation, and simulation of bounded semiring dynamic programs.  Motivated by recent toric geometry of ReLU networks, we further interpret tropical head decisions as active faces of Newton polytopes and normal fans, giving compact diagnostics for margins, bends, and future toric auxiliary losses.  A separate OpenAI Parameter-Golf track projects byte chunks to random-order graph positions and trains a dense recurrent micro language model with tied embeddings, toric position phases, tropical-ring attention, compact noncommutative-toric memory, strict-prefix GFlowNet graph-of-thought actions, embedding-space Forest-of-Thought search, BigramHash/CaseOps byte features, SmearGate confidence, stripped multi-token auxiliary heads, score-first test-time bias adaptation, and bit-packed 6-bit row-quantized export.  Its hybrid byte-TokenGT objective keeps official BPB as a byte-level prequential log score while applying causal or noncausal graph supervision to hidden reveal trajectories.  The new Forest-of-Thought head lifts Tree-of-Thought, Graph-of-Thought, and Forest-of-Thought test-time scaling into training-only hidden-state forests with sparse tree activation, UCB expansion pressure, dynamic self-correction, consensus-guided leaves, and trajectory-balance losses.  We also describe behavior-modification protocols in which learned GraphCG concept axes, toric-tropical chambers, and finite commutative-algebra certificates constrain hidden graph-of-thought trajectories to policy corridors for helpfulness, accuracy, tone, rigor, and detail.  The result is a small, auditable model design whose reasoning behavior is inspected through equivariance errors, active faces, toric moments, artifact bytes, BPB, corridor occupancy, GFlowNet diversity, and FoT consensus diagnostics rather than scalar accuracy alone.
\end{abstract}
...
\begin{figure}[t]
  ...
  \caption{ToricGT combines graph tokenization, tropical ring attention, noncommutative-torus phase channels, Soft-MoE routing, embedding-space GFlowNet fine-tuning, and training-only Forest-of-Thought search.}
...
\paragraph{Tropical-to-toric embedding.}
The more precise construction is a finite sidecar, not a claim that every Transformer layer is toric.  Let $N\simeq\Z^r$, $M=\operatorname{Hom}(N,\Z)$, $T_N=\operatorname{Spec}K[M]$, and let a rationalized tropical head provide characters $a_r\in M$ and scores
\begin{equation}
  \ell_r(u)=\ip{a_r}{u}+b_r,\qquad u=P h\in N_\R.
\end{equation}
Over a nonarchimedean field with uniformizer $\tau$, define
\begin{equation}
  f=\sum_r \tau^{-b_r}\chi^{a_r}.
\end{equation}
With the max-plus sign convention, $\operatorname{in}_{-u}(f)$ keeps exactly those monomials whose indices lie in $\argmax_r\ell_r(u)$.  Thus attention provenance is an initial degeneration; the tie locus is the tropical hypersurface, while the unique-max chambers are the surrounding normal-fan decision regions.  For a family of heads one forms an ideal $I_H\subset K[M]$ and chooses a rational fan $\Sigma$ refining the observed normal and Groebner decompositions.  The toric variety $X_\Sigma$ then supplies orbit strata, invariant divisors, Chow/Minkowski weights, monomial initial ideals, Cox modules, and Klyachko-style vector-bundle filtrations for exact audits \cite{maclagan2012intro,maclaganrincon2018tropicalideals,maclaganrincon2020balanced,m2tropical2019,khanmaclagan2024tropicalbundles}.  If $|\Sigma|=\operatorname{Trop}(V(I_H)\cap T_N)$ and the usual multiplication map is flat and surjective, the closure is a tropical compactification \cite{tevelev2007compactifications}; otherwise $\Sigma$ is a reproducible finite toric audit envelope, not a proof of global compactification.  Each sidecar records its lattice basis, denominator scale, coefficient valuation convention, max-plus/min-initial sign convention, CAS backend, and exact-or-surrogate status, and its losses remain late-phase auxiliaries behind BPB.
...
+\lambda_{\mathrm{FoT}}\mathcal L_{\mathrm{FoT}}.
\end{split}
\end{equation}
...
\paragraph{Embedding-space Forest-of-Thought.}
FoT generalizes single-chain reasoning by maintaining several search trees, sparsely activating promising paths, correcting flawed partial solutions, and selecting leaves by consensus \cite{bi2024forest}.  ToricGT implements the same pattern in hidden space rather than prompt space, while retaining the continuous/hybrid trajectory-balance perspective of GFlowNets \cite{lahlou2023continuous}.  For hidden positions $h_i$, a learned forest head produces tree activations $a_{it}$, values $v_{it}$, rewards $r_i$ derived from local NLL improvement and auxiliary geometric signals, and consensus logits $c_i$.  The compact loss is
\begin{equation}
  \mathcal L_{\mathrm{FoT}}
  =
  \lambda_{\mathrm{sp}}\mathcal L_{\mathrm{sparse}}
  +\lambda_{\mathrm{ucb}}\mathcal L_{\mathrm{UCB}}
  +\lambda_{\mathrm{corr}}\mathcal L_{\mathrm{correct}}
  +\lambda_{\mathrm{cons}}\mathcal L_{\mathrm{consensus}}
  +\lambda_{\mathrm{tb}}\mathcal L_{\mathrm{TB}}
  +\lambda_{\mathrm{sub}}\mathcal L_{\mathrm{SubTB}} .
\end{equation}
The terms respectively encourage sparse active trees, UCB-like expansion toward high-value uncertain branches, correction vectors aligned with lower-NLL successor states, consensus among high-reward leaves, and flow consistency.  The head is training-only in the Parameter-Golf track: it may shape hidden states and test-time scaling policies, but it is omitted from byte-accounted export unless a later ablation shows net BPB gain under the artifact cap.
...
FoT consensus/diversity & overconfident single-tree reasoning or useless branching \\
...
The live OAI-baseline adaptation refines this construction without inserting
extra scored tokens.  FineWeb tokens remain the scored SentencePiece sequence,
but the main model now receives deterministic low-rank TokenGT-style node
identifiers, endpoint-pair features, local one-dimensional-edge features, and
toric phase features.  Local causal edges are also represented internally as
virtual edge-token states and folded back into node hidden states by a gated
graph-output adapter.  For OAI FineWeb only, this graph-valued output is
flattened back to the original sequence order and corrected by a small
score-correction gate before BPB is computed.  General graph data remains
graph-structured and is not flattened by default.  The current adaptive
controller treats the identifier, endpoint, edge-token, virtual-edge, and
score-correction weights as separate hyperparameters rather than a single
``graph pressure'' knob, so later 1.5K-step restarts can determine which part
of the graphification helps or hurts early BPB.
...
The current tokenizer experiment replaces SP1024 in this same scored contract
by ConvexTok-2048 Det.  ConvexTok represents tokenisation as a byte-boundary
DAG: vertices are byte offsets, free one-byte edges guarantee feasibility, and
priced substring edges carry vocabulary colours.  Training solves a sparse LP
relaxation on tokenizer-training shards and rounds colour variables to a
deployable vocabulary; inference then encodes each string by exact min-plus
shortest-path dynamic programming.  This makes tokenizer choice a tropical
object rather than an opaque preprocessing step.  ToricGT logs the rounded
path length, LP lower bound, regret gap, active-edge entropy, top-two path
margins, and vocabulary-face occupancy.  The selected priced-token incidence
vectors define a toric vocabulary polytope shadow, while changes in the
shortest token path correspond to normal-fan wall crossings.  The first-class
TokenGT stream can consume LP score, rounded rank, byte length, and priced/free
edge flags as compact structural embeddings; a tiny toric regularizer aligns
token embeddings with these vocabulary-face coordinates.  BPB remains the
byte-denominated prequential score on the final flattened token sequence.

The ToricBLM scale-up path keeps this tokenizer-as-graph contract but moves to
a fresh ConvexTok-8192 biomedical vocabulary rather than reusing the
ConvexTok-2048 vocabulary or returning to BPE.  The 8192 vocabulary retains
byte fallback and reserves biomedical/control seed strings for UniProt-style
annotations, protein and nucleotide motifs, PDB/AFDB/ESMFold structure tags,
chemical graph strings, atomistic trajectory metadata, toric/tropical
diagnostics, and Forest-of-Thought control tokens.  A post-curation audit on
the new biological graph/FoT records found a 7.21\% token-count reduction
relative to ConvexTok-2048 on audited fields, but also a high fallback rate on
raw sequence strings; therefore the immediate full run keeps ConvexTok-8192
and defers larger motif-aware vocabularies until coordinate, residue-pair,
atom-type, chain-id, and dynamics fields are present.  The matching
mu-transfer model uses width $1536$, nine layers, twelve attention heads, six
KV heads, tied embeddings, and about $1.7\times10^8$ parameters.

For biomedicine preparation, the graph stream now includes a source-grounded
ToricBLM Forest-of-Thought dataset derived from local UniProt, UniRef50, DNA,
Rfam, RNAcentral, and PubChem SELFIES mirrors.  Each row stores
\texttt{graph\_json}, \texttt{thought\_forest\_json}, \texttt{convextok\_dag\_json},
\texttt{training\_views\_json}, and \texttt{enrichment\_status\_json}.  The
split is leakage-aware across sequence sketches, function labels and text
shingles, structure lookup hooks, graph histograms, and FoT topology.  The
current slice has $2788$ train, $156$ validation, and $128$ test records.  It
trains sequence/function/chemistry/structure-hook association.  A separate
AFDB v6 coordinate shard supplies true AlphaFold mmCIF-derived structure
targets: $256$ coordinate-bearing records split into $239$ train, $7$
validation, and $10$ test examples.  These records carry real CA/backbone
coordinates, coordinate masks, pLDDT, and graph/FoT structure context.  The
adapted OAI baseline therefore trains coordinate-native rectified-flow,
contact-map, distogram, and centered-RMSD structure losses on real coordinate
batches; rows without usable coordinates are skipped rather than replaced by
proxy structure targets.
