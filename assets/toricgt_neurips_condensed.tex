...
The \texttt{oai} branch also adds GraphCG-style analogical geometry without changing the exported byte model.  Let $h_t$ be a prefix-visible hidden state and let $a_t=(h_{t+s}-h_t)/(\|h_{t+s}-h_t\|+\epsilon)$ be a normalized relation arrow.  Repeated coarse byte-relation classes are encouraged to share a lattice basis $B\in\mathbb R^{d\times r}$ and to satisfy parallelogram closures.  For scale robustness, each relation class builds a nested soft Vietoris--Rips filtration
\begin{equation}
...
\end{equation}
so the intended object is a small persistence module over hidden-state analogy arrows \cite{edelsbrunner2002topological,zomorodian2005computing,cohensteiner2007stability}.  The implementation uses differentiable persistence surrogates rather than exact barcode backpropagation.  A radius-parametrized HDBSCAN gate further computes core distances $c_i$, mutual reachability $d_{\rm mr}(i,j)=\max\{c_i,c_j,d_{ij}\}$, and persistent density affinities across the same radii.  Only high-stability neighbors receive pullback weight; outlier arrows mostly stop contributing.  Periodic analyses render these noncommutative nested-complexes and HDBSCAN density sweeps as directed filtration curves and heatmaps next to 3D graph-of-thought trajectories, Ramachandran-style phase plots, energy landscapes, and reasoning/K/BPB simplices.
