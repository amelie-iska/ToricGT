...
- The requested "there be dragons" responding-onlooker ablation is satisfied in
  executable paths by absence: repository search currently finds no matching
  prompt role or observer path outside ignored outputs/checkpoints/data.
- Skeptical-onlooker rule: an advanced geometric object may affect training
  only if it has a logged metric, a documented ablation, and a plot generated by
  the analysis suite. Otherwise it remains a diagnostic.
