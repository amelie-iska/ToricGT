...
## 2026-06-05 Step 6250 Seq4096 Stableadv BPB Review

Context:

```text
run: toricgt_seq4096_oai_bpb1p2085_stableadv_20260604T235447Z
W&B: amelie-iska-math/toricgt-parameter-golf/toricgt_seq4096_oai_bpb1p2085_stableadv_20260604T235447Z
analysis: outputs/post_resume_analysis/toricgt_seq4096_oai_bpb1p2085_stableadv_20260604T235447Z/step-00006250
checkpoint: amelie-iska/parameter-golf/checkpoints/toricgt_seq4096_oai_bpb1p2085_stableadv_20260604T235447Z/toricgt_seq4096_oai_bpb1p2085_stableadv_20260604T235447Z_step_006250.pt
loop: parameter_golf_bpb_target, iteration 5 / 100, target BPB <= 1.2
```

The W&B history export still lagged at step 5000, but the W&B summary,
checkpoint-local log export, synopsis, and BPB reports covered step 6250.
The active loop primary signal was `openai_parameter_golf/bpb =
1.2136128495545466` at step 6250.  Best validation/FineWeb-style BPB in this
loop remains `1.2085`, so the active primary gap is `0.0136128496` and the
best-gate gap is `0.0085`.  The sampled local official-style OAI evaluator was
available but weaker: `oai_competition/bpb = 1.4154305979` on 64 sampled
seq-len-256 FineWeb/SP1024 sequences.  That sampled probe is a calibration
warning and control requirement, not a target-clearing artifact.

Metric and plot categories:

- Desired: validation BPB is still descending after the post-4000 rebound:
  `1.2323 -> 1.2275 -> 1.2241 -> 1.2217 -> 1.2198 -> 1.2169 -> 1.2148 ->
  1.2136` over steps 4500--6250.  Train BPB continues to cross the target
  intermittently (`1.1969` at 6000, `1.1216` at 6200, `1.2035` at 6250).
  GraphCG Gram plots are close to diagonal, exact trajectory-proxy Koszul
  `d2` residual is zero, selected BGG/Gale consistency is high, Slepian
  concentration is strong in the full diagnostic payload, the checkpoint is
  under the artifact limit, and the BPB/simplex/3D/phase/energy/topology/
  tropical plots are present and noncollapsed.
- Desired but too weak or slow: the validation slope is shallow
  (`-0.00093` BPB per 100 steps in the synopsis, `-0.00048` recent velocity in
  the full diagnostic payload), projected target step is about `7164`, and the
  automatic categories remain broad: desired `57`, weak-or-slow `46`,
  undesirable `16`.  MST efficiency is modest (`mean 0.2017`), trajectory
  smoothness is low for the memory surface, R4 path length is large
  (`1784.69`), analogical transport accuracy is still zero, and GFlowNet
  branch replay/test-time-scaling metrics are not directly available.
- Undesirable: the sampled official-style OAI/FineWeb evaluator is far above
  target, GraphCG basis condition is extreme on the memory graph, global BGG
  standard leakage is high (`0.52265`) despite good selected-record consistency,
  toric active-face margin is negative (`-2.0026`), toric geometry/binomial
  losses are high, tropical chamber crossing is frequent (`mean 0.9204`), and
  advanced aux/Koszul/toric losses drifted upward in the W&B metric table.

Plot review:

- BPB dashboard and phase-plane plots show noisy train BPB with several
  sub-target dips while validation descends smoothly but slowly.  There is no
  positive first derivative at step 6250 and no visual floor-bounce basin.
- The simplex/tetrahedron plots cluster nearer low-K/low-BPB than high-MST
  efficiency.  This is acceptable for a pre-threshold compression phase, but
  not strong enough to promote reasoning sidecars above BPB.
- 3D memory trajectories and energy landscapes are active and branched, with a
  low-energy cluster plus high-energy ridges and long jumps.  They support
  sidecar monitoring, not a heavier memory/GFlowNet scalar.
- Ramachandran-style phase/energy and tropical chamber plots show broad phase
  winding and frequent active-face changes.  Because `tropical/bpb_plateau`
  pressure is zero at the gate, this does not justify rollback.
- GraphCG plots are structurally clean by Gram shape but ill-conditioned by
  scale.  BGG/Koszul plots are locally consistent, but global standard leakage
  keeps the reasoning gate guarded.

Mathematical and statistical interpretation:

- The recent validation first differences over 250-step checkpoints are
  negative: `4750->5000: -0.0034`, `5000->5250: -0.0024`,
  `5250->5500: -0.0019`, `5500->5750: -0.0029`, `5750->6000: -0.0021`,
  `6000->6250: -0.0012`.  Recent second differences are small and mixed
  (`+0.0010`, `+0.0005`, `-0.0010`, `+0.0008`, `+0.0009`), and no Hessian
  trace/sharpness probe is available.  This is decelerating descent, not a
  rollback-grade positive-derivative basin.
- The two-gate rule supports continuing the BPB-clean schedule.  FineWeb/
  openai_parameter_golf BPB is close but not at target; hard-reasoning,
  topology, GraphCG, toric/tropical, memory, BGG, and Kolmogorov/relative-K
  proxies are alive but guarded.  Since the gates do not show a clean
  contradiction, no mixture-ratio or auxiliary-weight edit is warranted.
- The sampled official-style FineWeb result prevents overclaiming OOD transfer.
  Any strong FineWeb result still needs fixed-slice controls against tokenizer
  convention, n-gram/bigram-bias artifacts, dataset easiness, random-init and
  FineWeb-only baselines, hard-data zero-shot comparison, and sidecar ablations.
- GFlowNet graph-of-thought quality, branch replay, branch BPB, and
  test-time-scaling diagnostics are planned but absent in the current metrics.
  They remain reasoning-gate requirements, not BPB-gate evidence at this step.

`scripts/propose_training_adjustments.py` was rerun on this analysis directory
as `training_adjustment_proposal_codex_rerun.{json,md}` with `target_bpb=1.2`,
`checkpoint_step=6250`, and `gate_step=20000`.  Under the active loop horizon
it classified the gate as `on_track` and proposed
`hold_bpb_clean_schedule` with no restart.  I used that as evidence, not
authority.  The script currently expects `oai_competition/summary.json`, while
this analysis wrote `oai_competition/seq4096_summary.json`, so its OAI fields
were `NaN`; the sampled OAI/FineWeb value above was reviewed manually.
...
```text
selected checkpoint: amelie-iska/parameter-golf/checkpoints/toricgt_seq4096_oai_bpb1p2085_stableadv_20260604T235447Z/toricgt_seq4096_oai_bpb1p2085_stableadv_20260604T235447Z_step_006250.pt
config: config/train.parameter_golf_random_order_dense.yaml
training/config edits: none
training tmux: toricgt_seq4096_oai_bpb1p2085_stableadv_20260604T235447Z
training process: active original Seq4096 trainer records/track_10min_16mb/2026-03-19_TrainingOptSeq4096/train_gpt.py
active observed step after review: at least 6350 / 20000
training log: amelie-iska/parameter-golf/logs/toricgt_seq4096_oai_bpb1p2085_stableadv_20260604T235447Z.txt
checkpoint dir: amelie-iska/parameter-golf/checkpoints/toricgt_seq4096_oai_bpb1p2085_stableadv_20260604T235447Z
...
supervisor: no active scripts/supervise_parameter_golf_training.py process found; active trainer, W&B mirror, full diagnostics, nan guard, and CPU sidecar were preserved
legacy interrupting watcher: toricgt_seq4096_oai_bpb1p2085_stableadv_20260604T235447Z_analysis stopped after this decision
non-pausing generic watcher: toricgt_watch_training_analysis_stableadv_6500_20260605T022634Z
generic watcher script: scripts/watch_training_analysis.py, CPU/fp32, target step 6500, no --pause-training-before-analysis
generic watcher log: outputs/post_resume_analysis/toricgt_seq4096_oai_bpb1p2085_stableadv_20260604T235447Z/sidecar_logs/toricgt_watch_training_analysis_stableadv_6500_20260605T022634Z.log
generic watcher output root: outputs/post_resume_analysis/toricgt_seq4096_oai_bpb1p2085_stableadv_20260604T235447Z_watch_training_analysis_sidecar
stop sentinel: not written; no better replacement strategy was established
```

The preserved BPB loop environment is:

```text
BPB_TARGET=1.2
BPB_MAX_REVIEW_ITERATIONS=100
BPB_LOOP_STATE=/home/iska/Documents/amelie/bio/ToricGT/outputs/toricgt_seq4096_oai_bpb1p2085_stableadv_20260604T235447Z_bpb_codex_loop_state.json
BPB_LOOP_STOP_FILE=/home/iska/Documents/amelie/bio/ToricGT/outputs/toricgt_seq4096_oai_bpb1p2085_stableadv_20260604T235447Z_bpb_codex_loop_stop
...
```text
analyzed run: amelie-iska-math/toricgt-parameter-golf/toricgt_seq4096_oai_bpb1p2085_stableadv_20260604T235447Z
analyzed checkpoint: amelie-iska/parameter-golf/checkpoints/toricgt_seq4096_oai_bpb1p2085_stableadv_20260604T235447Z/toricgt_seq4096_oai_bpb1p2085_stableadv_20260604T235447Z_step_004500.pt
analysis: outputs/post_resume_analysis/toricgt_seq4096_oai_bpb1p2085_stableadv_20260604T235447Z/step-00004500
...
```text
loop primary BPB: openai_parameter_golf/bpb = 1.232315136329848 at step 4500
loop target gap from current primary BPB: 0.03231513632984795
best W&B FineWeb/openai_parameter_golf BPB in analyzed history: 1.2084842981821384
best validation target gap: 0.008484298182138472
synopsis best validation BPB: 1.2085
step-4500 validation BPB/loss: 1.2323 / 2.0807
step-4526 low-train trigger validation BPB/loss after analysis: 1.2327 / 2.0814
step-4500 train BPB/loss: 1.3342 / 2.2546
best train BPB in analyzed W&B export: 1.1118309382332459
sampled compact OAI BPB/loss: 1.4307509809020849 / 2.445587396621704
sampled compact OAI scope: 64 sampled 256-token sequences, not authoritative
metric categories: desired=36, weak_or_slow=16, undesirable=17
artifact probe: 15,979,507 bytes after int8+zlib; 20,493 bytes under 16 MB
```

Metric and plot categorization:

- Desired: the best FineWeb/openai_parameter_golf validation BPB remains close
  to the `1.2` target; validation BPB is descending again from the post-4000
  rebound (`4250 -> 4500: -0.0040 BPB`, about `-0.0016 BPB/100 steps`);
  train samples still intermittently cross the target; checkpoint export is
  under the 16 MB limit; GraphCG, topology, toric/tropical, Slepian/Pollak,
  Koszul/BGG, analogical, and complexity diagnostics are present; exact
  persistence has small chain residuals and chart coverage is complete.
- Desired but too weak or slow: current primary BPB is still `0.0323` above
  target; the useful best validation BPB is still `0.0085` above target;
  transfer efficiency is not stable because train drops are noisy and only
  weakly become validation drops; GraphCG basis separation is good for token
  and norm records but condition numbers remain high; MST efficiency is useful
  only for the embedding records and weak for layer/attention records; path
  smoothness is low across records; BGG/Koszul exactness is good but standard
  leakage and fitting/rank residuals remain visible; Slepian concentration is
  weak in the curve diagnostic payload.
- Undesirable: validation rebounded sharply from the step-4000 low to the
  4154--4250 shelf; the sampled compact OAI probe is worse than the W&B
  FineWeb/openai_parameter_golf signal and is sampled, so it cannot promote a
  checkpoint; `advanced/backprop_enabled` and `advanced/runtime_scale_applied`
  entered the undesirable bucket because advanced sidecars turned on at step
  4500; toric active-face margin is negative (`-2.0992`), toric geometry loss
  is high (`8.1798`), tropical chamber crossing rate is high (mean about
  `0.8833`), analogical transport accuracy is zero in the geometry records,
  and no Hessian/sharpness or GFlowNet replay time series is available.

Plot review:

- BPB dashboards, velocity, phase-plane, target-zone, ETA, transfer, and
  rockfall plots show a rebound after the step-4000 low followed by a slow
  validation descent.  The old 4K gate-velocity plot marks the run off-track
  because the gate is already past; for this review the active loop target is
  still `BPB <= 1.2`, so that stale 4K shortfall is evidence of slowness, not
  an automatic rollback trigger.
- BPB control and evidence maps recommend `bpb_gap=1.0` and sidecar-only
  structural families.  The evidence policy is
  `monitor_metric_context_keep_bpb_primary`, with toric/Slepian the dominant
  current structural pressure.
- Simplex and tetrahedron plots show coherent but uneven reasoning geometry:
  useful GraphCG and embedding structure on R0/R1, weaker low-K/MST corners,
  and no robust low-BPB/high-MST/high-smoothness basin.
- 3D trajectory, phase, winding, and energy-landscape plots show noncollapsed
  but noisy paths.  R4 bigram-memory has especially long trajectory length
  (`1539.68`) and low path smoothness (`0.0314`).
- Directed filtration, noncommutative heatmap, persistence, and
  commutative-algebra audit plots show useful topology with tiny directed cycle
  flux, but standard leakage and fitting/rank residuals keep the reasoning
  gate guarded.
- GraphCG basis plots are visually close to diagonal; analogical maps remain
  weak; tropical chamber plots show frequent active-face transitions and
  plateau bursts, matching the guarded structural-recature reports.

Mathematical/statistical interpretation:

- The BPB gate is close but not satisfied.  Let `L_val` be the FineWeb-style
  validation cross-entropy and `BPB = L_val / log(2) / bytes_per_token`.  The
  best observed W&B validation BPB (`1.20848`) is only `0.7%` above target, but
  the current primary value (`1.23232`) is `2.7%` above target.  The
  competition gate should therefore remain the dominant objective.
- Validation finite differences are mixed, not a clean floor-bounce signal:
  `4000 -> 4154: +0.0278`, `4154 -> 4196: +0.0007`,
  `4196 -> 4224: -0.0004`, `4224 -> 4250: -0.0003`,
  `4250 -> 4500: -0.0040`, `4500 -> 4526: +0.0004`.  Late second differences
  include a small positive value after 4500, but there is no Hessian trace,
  sharpness probe, or sustained positive first derivative sufficient to justify
  rollback.
- The train/validation gap is negative at the validation points because train
  BPB is highly sample-dependent.  Low train BPB checkpoints at 4154, 4196,
  4224, and 4526 are useful artifact candidates, but their paired validation
  BPB stays around `1.232--1.237`, so they are not sufficient promotion
  evidence.
- The sampled compact OAI result should be treated as an auxiliary probe only:
  it uses a sampled 64-sequence, 256-token scope and a 1024-BPE Seq4096 path.
  It is not a full official-style validation result and does not override
  W&B FineWeb/openai_parameter_golf BPB.
- A transfer claim is not warranted yet.  The current run is directly exposed
  to FineWeb SP1024 training shards, so any strong FineWeb result needs
  controls against tokenizer convention, n-gram/bigram bias, dataset easiness,
  FineWeb-only training, hard-data-only zero-shot evaluation, and matched-budget
  ablations before being interpreted as OOD transfer from hard reasoning data.
- Direct curated hard-reasoning BPB and held-out GoT/ToT/CoT validation BPB are
  not present in this analysis.  The reasoning gate is therefore represented by
  checkpoint geometry and sidecar diagnostics, not by a direct curated BPB time
  series.
- GFlowNet branch replay and test-time-scaling metrics are not present as
  time-series evidence.  The `toric_gfn_bpb` tetrahedron is a geometry proxy;
  it is insufficient to drive a rollback or scalar edit.
- Kolmogorov/relative-K proxies are mixed: relative-K ranges from about
  `0.65` to `0.81`, but recent log NCD is high (`0.9001`).  This supports
  preserving complexity diagnostics while keeping BPB primary.
- GraphCG is useful but not clean: R0/R1 disentanglement is high
  (`0.9487`/`0.9311`), R4 is acceptable (`0.7820`), while layer/attention
  records are weaker (`0.4680`/`0.5177`) and all condition numbers are large.
- Koszul/BGG is internally coherent enough to preserve: geometry records have
  `koszul_d2_residual=0`, BGG resolution consistency is near `1.0`, and the
  curve payload has `resolution_consistency=0.9811`; however payload
  `gale_dual_consistency=0`, `standard_leakage=0.5664`, signature smoothness
  `0`, Buchsbaum-Eisenbud rank residual `0.3130`, and fitting rank residual
  `0.1029` make it a guarded sidecar.
- Toric/tropical behavior is structurally active but too noisy for heavier
  loss: toric active-face entropy is `0.6642`, shadow fan entropy is `0.8447`,
  active-face margin is negative, and chamber crossing rates are high.  This
  explains the controller's toric/Slepian pressure without implying an
  immediate toric-weight increase.

Adjustment proposal:

```text
/home/iska/miniconda3/bin/conda run --no-capture-output -n tokengt env PYTHONPATH=src \
  python scripts/propose_training_adjustments.py \
  --analysis-dir outputs/post_resume_analysis/toricgt_seq4096_oai_bpb1p2085_stableadv_20260604T235447Z/step-00004500 \
  --output-json outputs/post_resume_analysis/toricgt_seq4096_oai_bpb1p2085_stableadv_20260604T235447Z/step-00004500/training_adjustment_proposal.json \
  --output-md outputs/post_resume_analysis/toricgt_seq4096_oai_bpb1p2085_stableadv_20260604T235447Z/step-00004500/training_adjustment_proposal.md \
  --target-bpb 1.2 --checkpoint-step 4500
```

The first rerun attempt under base Python failed because `torch` was not
installed there; the command above succeeded under `tokengt`.  The proposal
recommended `restart_from_best_checkpoint_with_damped_structural_sidecars`
because it evaluates against the already-missed 4K gate and labels the current
status `off_track`.  I used that as evidence of slowness, not authority.  For
the active 100-iteration BPB loop, validation is still descending, the
reasoning gate is guarded but coherent, and no derivative/Hessian evidence
identifies a floor-bounce basin.  A restart would duplicate an active run and
discard useful continuation evidence.

Decision and active handoff:

```text
action: CONTINUE
reason: BPB target not reached, but validation BPB is still descending and no
        decisive rollback/edit signal is present.
code/config edits: none
better-strategy stop sentinel: not written
training active after review: yes, observed through step 4550 while reviewing
training tmux: toricgt_seq4096_oai_bpb1p2085_stableadv_20260604T235447Z
training process: /home/iska/miniconda3/envs/tokengt/bin/python -u records/track_10min_16mb/2026-03-19_TrainingOptSeq4096/train_gpt.py
training log: amelie-iska/parameter-golf/logs/toricgt_seq4096_oai_bpb1p2085_stableadv_20260604T235447Z.txt
checkpoint dir: amelie-iska/parameter-golf/checkpoints/toricgt_seq4096_oai_bpb1p2085_stableadv_20260604T235447Z
analyzed checkpoint: amelie-iska/parameter-golf/checkpoints/toricgt_seq4096_oai_bpb1p2085_stableadv_20260604T235447Z/toricgt_seq4096_oai_bpb1p2085_stableadv_20260604T235447Z_step_004500.pt
latest observed checkpoint after analysis: amelie-iska/parameter-golf/checkpoints/toricgt_seq4096_oai_bpb1p2085_stableadv_20260604T235447Z/toricgt_seq4096_oai_bpb1p2085_stableadv_20260604T235447Z_step_004526.pt
W&B run: https://wandb.ai/amelie-iska-math/toricgt-parameter-golf/runs/toricgt_seq4096_oai_bpb1p2085_stableadv_20260604T235447Z
supervisor: no active scripts/supervise_parameter_golf_training.py process was observed; preserved the active training tmux, mirror, diagnostics, and nan-guard processes instead of restarting.
```

The prior `watch_seq4096_analysis.py` review watcher was stopped after this
decision to avoid a duplicate interrupt-style review loop.  The replacement
sidecar is non-pausing and uses the requested generic watcher:

```text
watcher tmux: toricgt_watch_training_analysis_stableadv_4750
watcher script: scripts/watch_training_analysis.py
watcher target: checkpoint >= 4750
watcher log: logs/toricgt_seq4096_oai_bpb1p2085_stableadv_20260604T235447Z.watch_training_analysis_4750.txt
watcher output root: outputs/post_resume_analysis/toricgt_seq4096_oai_bpb1p2085_stableadv_20260604T235447Z
watcher device/precision: cpu / fp32
pause behavior: non-interrupting; no --pause-training-before-analysis
Codex review hook: enabled as a sidecar only; training remains active while reviews run
```

Loop environment preserved for the replacement watcher:
...
## 2026-06-05 Step 6000 Seq4096 Stableadv BPB Review
...
run: toricgt_seq4096_oai_bpb1p2085_stableadv_20260604T235447Z
W&B: amelie-iska-math/toricgt-parameter-golf/toricgt_seq4096_oai_bpb1p2085_stableadv_20260604T235447Z
analysis: outputs/post_resume_analysis/toricgt_seq4096_oai_bpb1p2085_stableadv_20260604T235447Z/step-00006000
checkpoint: amelie-iska/parameter-golf/checkpoints/toricgt_seq4096_oai_bpb1p2085_stableadv_20260604T235447Z/toricgt_seq4096_oai_bpb1p2085_stableadv_20260604T235447Z_step_006000.pt
loop: parameter_golf_bpb_target, iteration 4 / 100, target BPB <= 1.2
```

The W&B metrics export reported the run as `running` but lagged at history step
5000; the checkpoint-local logs, synopsis, and BPB reports covered step 6000.
The active loop primary signal was `openai_parameter_golf/bpb = 1.2148427034`
at step 6000, improving from `1.2323151363` at step 4500 and `1.2275301955`
at steps 4750/5000. Best validation BPB in the step-6000 analysis was
`1.2085`, leaving a target gap of `0.0085` for the best gate value and
`0.01484` for the loop primary value.

Metric and plot categories:

- Desired: the main BPB trajectory is descending after the post-4000 bounce;
  validation BPB moved `1.2323 -> 1.2275 -> 1.2241 -> 1.2217 -> 1.2198 ->
  1.2169 -> 1.2148` over steps 4500--6000. Train BPB frequently crosses the
  target, including `1.1969` at step 6000. Slepian concentration is excellent
  in the geometry proxy for R0/R1/R4, BGG/Gale consistency is high in the
  checkpoint geometry records, Koszul `d2` is zero in that proxy, GraphCG
  off-diagonal coherence is near zero, and the generated 3D, phase, simplex,
  topology, tropical, and energy plots are nonblank and structurally active.
- Desired but too weak or slow: the validation descent velocity is small
  (`~0.00084` BPB per 100 steps in the last interval, `~0.00099` recent
  regression slope per 100 steps), train-to-validation transfer efficiency is
  weak (`0.03684` recent), and automatic metrics put 46 metrics in this band.
  MST efficiency is modest (`0.34`, `0.36`, `0.06`, `0.07`, `0.18` across
  R0--R4), path smoothness is low for R2/R3/R4, R4 memory trajectory length is
  large (`1796`), and analogical transport accuracy is still `0.0`.
- Undesirable: 16 automatic metrics were undesirable, mostly auxiliary/runtime
  or target-gap symptoms. Advanced aux loss, GraphCG loss, Koszul/BGG loss,
  toric-tropical loss, and weighted unscaled loss rose in the W&B export.
  The sampled local `oai_competition` eval was `1.4119556605` on a 64-sequence,
  seq-len-256 FineWeb/SP1024 slice, so it is a calibration warning rather than
  the authoritative loop metric. High tropical chamber crossing rates
  (`~0.85--0.96` on several records), high GraphCG basis condition numbers, and
  high R4 curvature are guard signals, not reasons to discard the checkpoint.

Mathematical and statistical interpretation:

- The primary BPB finite differences do not show a hard floor-bounce basin.
  After the 4000-to-4154 bounce, validation first differences were mostly
  negative; the final intervals were `-0.0024`, `-0.0019`, `-0.0029`, and
  `-0.0021` BPB per checkpoint. Second differences at 5000--6000 alternated
  small signs (`+0.0018`, `+0.0010`, `+0.0005`, `-0.0010`, `+0.0008`), with no
  Hessian/sharpness probe artifact available in this analysis directory.
- The two-gate rule supports continuing. The competition/FineWeb-style BPB gate
  is not at target, but it is improving; the reasoning gate has pressure but no
  collapse. If later gates disagree, the correct response remains mixture-ratio
  or auxiliary-weight adjustment, not dropping the hard-reasoning objective.
- The strong FineWeb-like BPB after limited exposure remains only possible OOD
  transfer. It still requires controls against tokenizer convention, n-gram or
  bigram-bias effects, and dataset easiness: fixed FineWeb slices, random-init
  and FineWeb-only baselines, ToricGT-hard-data zero-shot comparison, and
  ablations for GFlowNet/GraphCG/toric/topology sidecars.
- GFlowNet graph-of-thought and branch/test-time-scaling diagnostics were not
  independently decisive in this analysis. The simplex and trajectory proxies
  indicate preserved graph/trajectory structure, but BPB remains the primary
  pre-threshold optimization signal.

`scripts/propose_training_adjustments.py` was rerun on the step-6000 analysis
directory with `target_bpb = 1.2`. It again classified the gate as `off_track`
and recommended `restart_from_best_checkpoint_with_damped_structural_sidecars`,
primarily because the historical 4K gate had already been missed. I used that
as evidence, not authority: the active loop is a 100-review BPB target loop, and
the post-bounce BPB derivatives support more same-config descent before a
rollback or scalar edit.
...
```text
selected checkpoint: amelie-iska/parameter-golf/checkpoints/toricgt_seq4096_oai_bpb1p2085_stableadv_20260604T235447Z/toricgt_seq4096_oai_bpb1p2085_stableadv_20260604T235447Z_step_006000.pt
config: config/train.parameter_golf_random_order_dense.yaml
code/config edits: none
training tmux: toricgt_seq4096_oai_bpb1p2085_stableadv_20260604T235447Z
training process: active original Seq4096 trainer records/track_10min_16mb/2026-03-19_TrainingOptSeq4096/train_gpt.py
training log: amelie-iska/parameter-golf/logs/toricgt_seq4096_oai_bpb1p2085_stableadv_20260604T235447Z.txt
checkpoint dir: amelie-iska/parameter-golf/checkpoints/toricgt_seq4096_oai_bpb1p2085_stableadv_20260604T235447Z
active analysis watcher: toricgt_seq4096_oai_bpb1p2085_stableadv_20260604T235447Z_analysis
active watcher script: scripts/watch_seq4096_analysis.py, non-pausing, target step 6250
non-pausing generic watcher: toricgt_watch_training_analysis_stableadv_6500_20260605T022634Z
generic watcher script: scripts/watch_training_analysis.py, CPU/fp32, target step 6500, no --pause-training-before-analysis
generic watcher log: outputs/post_resume_analysis/toricgt_seq4096_oai_bpb1p2085_stableadv_20260604T235447Z/sidecar_logs/toricgt_watch_training_analysis_stableadv_6500_20260605T022634Z.log
generic watcher output root: outputs/post_resume_analysis/toricgt_seq4096_oai_bpb1p2085_stableadv_20260604T235447Z_watch_training_analysis_sidecar
supervisor: no active scripts/supervise_parameter_golf_training.py process found; active Seq4096 trainer and watcher were preserved
stop sentinel: not written; no better replacement strategy was established
```
...
```text
requested analyzed run: amelie-iska-math/toricgt-parameter-golf/toricgt_seq4096_4k_recovery_r89_20260604T202239Z
requested analyzed checkpoint: amelie-iska/parameter-golf/checkpoints/toricgt_seq4096_4k_recovery_r89_20260604T202239Z/toricgt_seq4096_4k_recovery_r89_20260604T202239Z_step_003850.pt
requested analysis: outputs/post_resume_analysis/toricgt_seq4096_4k_recovery_r89_20260604T202239Z/step-00003850
current derivative analysis also inspected: outputs/post_resume_analysis/toricgt_seq4096_4k_recovery_r89_20260604T202239Z/step-00003900
loop: parameter_golf_bpb_target, iteration 1 / 100, target BPB <= 1.2
```
...
```text
step-3850 FineWeb-style validation BPB/loss: 1.2108 / 2.0444
step-3850 target gap: 0.0108
step-3850 train BPB evidence after resume: best observed train sample 1.1667, but no paired validation slope yet
step-3900 FineWeb-style validation BPB/loss: 1.2097 / 2.0426
step-3900 target gap: 0.0097
step-3900 validation slope from 3850 to 3900: -0.0022 BPB per 100 steps
required slope from 3900 to hit 1.2 by step 4000: -0.0097 BPB per 100 steps
projected target step from validation slope: 4340.9
sampled seq4096 OAI competition BPB/loss at 3850: 1.4055 / 2.4025
sampled seq4096 OAI competition BPB/loss at 3900: 1.4045 / 2.4007
sampled OAI eval scope: 64 local FineWeb sp1024 validation sequences, seq_len=256
step-3850 metric categories: desired=31, weak_or_slow=8, undesirable=0
step-3900 metric categories: desired=39, weak_or_slow=35, undesirable=6
structural recapture score: 0.0411 at 3850, 0.0396 at 3900, low band
dominant structural pressure: tropical_complexity in BPB reports; trajectory_memory in simplex proposal
artifact probe: under 16 MB but with tight margin
```

Metric and plot categorization:

- Desired: FineWeb-style validation BPB improved from `1.2108` to `1.2097`;
  validation loss also improved from `2.0444` to `2.0426`; train samples
  include below-target values; BGG/Koszul `d2` residual and standard leakage
  are zero in the W&B structural payload; generated BPB, velocity, simplex,
  3D trajectory, phase-energy, energy landscape, GraphCG, tropical, topology,
  and BGG/Koszul plots were present and inspectable.
- Desired but too weak or slow: validation BPB is descending, but the observed
  drop rate is only about `0.0022` BPB/100 steps while the gate needs about
  `0.0097` BPB/100 steps from step 3900.  Transfer evidence is still thin:
  step 3850 had no paired validation slope and step 3900 has only one paired
  validation interval.  GraphCG/BGG sidecar structure is coherent enough to
  keep, but the simplex/tetrahedron plots do not place the run in a strong
  low-BPB/high-MST-efficiency corner.
- Undesirable: the official-style full validation gate remains above target;
  the sampled OAI competition evaluator is much worse (`~1.405` BPB) and is a
  calibration warning even though it is sampled rather than authoritative.
  Tropical chamber/phase plots show active crossings and high plateau pressure,
  and complexity/Kolmogorov proxy pressure remains high (`NCD LZMA ~0.889`).
  GFlowNet graph-of-thought, branch replay, test-time scaling, Hessian trace,
  and sharpness probes were not available as decisive promotion signals in
  this handoff.

Mathematical/statistical interpretation:

- The first validation derivative is negative, so there is no rollback trigger
  from a positive first derivative.  The second difference is underpowered
  because there are only two recent validation points in the current r89 run,
  and no Hessian probe is available.
- The problem is velocity, not an observed floor bounce.  From step 3900, the
  target gap is `0.0097` BPB with 100 steps left.  The measured velocity would
  cross near step `4340.9`, giving a gate overrun of about `340.9` steps.
- The two-gate rule is preserved.  The BPB gate is primary and off-track; the
  reasoning gate is sidecar-level but not collapsing.  Because the gates do not
  justify discarding either objective, the intervention is a scalar BPB-velocity
  recapture: keep architecture/data/auxiliary families, hold matrix/scalar LR,
  and slightly raise tied-embedding LR.
- A strong FineWeb-style result after limited FineWeb exposure could indicate
  OOD transfer from hard reasoning data, but this run cannot claim that without
  controls against tokenizer effects, n-gram baselines, dataset easiness,
  FineWeb-only exposure, hard-only exposure, mixed-budget exposure, and
  auxiliary ablations.

The adjustment proposal was rerun on the requested analysis directory:

```text
/home/iska/miniconda3/envs/tokengt/bin/python scripts/propose_training_adjustments.py \
  --analysis-dir outputs/post_resume_analysis/toricgt_seq4096_4k_recovery_r89_20260604T202239Z/step-00003850 \
  --target-bpb 1.2 --gate-step 4000 --checkpoint-step 3850
```

It recommended `prepare_midrun_recovery_branch` with `restart_policy:
no_restart_until_next_gate`.  The same script on the current step-3900 analysis
recommended `restart_from_best_checkpoint_with_damped_structural_sidecars`
because the validation projection misses the 4K gate.  I used both as evidence:
the 3850 evidence alone would not justify a rollback, while the 3900 paired
validation slope justifies the supervisor's preemptive scalar recovery.

Decision: `EDIT_AND_RESTART`.

Reason: the active supervisor observed a projected 4K miss at step 3900 and
launched the smallest high-impact scalar restart from the best current
validation checkpoint.  The selected restart preserves the ToricGT
Parameter-Golf architecture and keeps random-order autoregressive decoding,
tropical/hybrid attention, toric memory, dense contest weights, GraphCG,
topology/toric/BGG diagnostics, Kolmogorov diagnostics, and hard reasoning
sidecar controls.  No JEPA, model-code, tokenizer, data, checkpoint deletion,
or artifact-publishing change was made.  No better-strategy stop sentinel was
written; the BPB loop remains active.
...
action: EDIT_AND_RESTART
active supervisor: scripts/watch_seq4096_4k_recovery.py
supervisor trigger: launching_preemptive_gate_risk_recovery from r89 step 3900
old training tmux: toricgt_seq4096_4k_recovery_r89_20260604T202239Z, now exited
active training tmux: toricgt_seq4096_4k_recovery_r90_20260604T203009Z
active training process: train_gpt.py verified running on GPU
active W&B run: amelie-iska-math/toricgt-parameter-golf/toricgt_seq4096_4k_recovery_r90_20260604T203009Z
active training log: amelie-iska/parameter-golf/logs/toricgt_seq4096_4k_recovery_r90_20260604T203009Z.txt
active checkpoint dir: amelie-iska/parameter-golf/checkpoints/toricgt_seq4096_4k_recovery_r90_20260604T203009Z
resume checkpoint: amelie-iska/parameter-golf/checkpoints/toricgt_seq4096_4k_recovery_r89_20260604T202239Z/toricgt_seq4096_4k_recovery_r89_20260604T202239Z_step_003900.pt
resume controls: reset_optimizer=0, reset_rng=0, reset_loader=0
r90 scalar delta: tied_embed_lr 0.034486 -> 0.03621
r90 held controls: matrix_lr=0.018, scalar_lr=0.018, train_batch_tokens=1048576,
                   graphcg=0.02, toric_tropical=0.003, slepian=0.01,
                   koszul_bgg=0.0001, analogy=0.00008
active supervisor tmux: toricgt_seq4096_4k_gate_r90_20260604T203009Z
active supervisor log: logs/toricgt_seq4096_4k_recovery_r90_20260604T203009Z.4k_gate.txt
existing compact analysis watcher: toricgt_seq4096_4k_analysis_r90_20260604T203009Z
requested generic non-pausing CPU watcher: toricgt_watch_training_analysis_r90_3950
generic watcher implementation: scripts/watch_training_analysis.py
generic watcher log: logs/toricgt_seq4096_4k_recovery_r90_20260604T203009Z.watch_training_analysis_3950.txt
generic watcher output root: outputs/live_periodic_reviews/toricgt_seq4096_4k_recovery_r90_20260604T203009Z_watch_training_analysis
generic watcher target: checkpoint >= 3950 on CPU, no --pause-training-before-analysis flag
loop env: BPB_TARGET=1.2, BPB_MAX_REVIEW_ITERATIONS=100,
          BPB_LOOP_STATE=/home/iska/Documents/amelie/bio/ToricGT/outputs/toricgt_seq4096_4k_recovery_r89_20260604T202239Z_bpb_codex_loop_state.json,
          BPB_LOOP_STOP_FILE=/home/iska/Documents/amelie/bio/ToricGT/outputs/toricgt_seq4096_4k_recovery_r89_20260604T202239Z_bpb_codex_loop_stop,
          BPB_LOOP_NAME=parameter_golf_bpb_target
```
