...
4. Add a controller rule: if `toric_cca_topology_loss` rises while BPB improves,
   ...
   weight.
## 2026-06-05 Periodic Symbolic-Resolution Certificate Artifacts

The Seq4096 periodic geometry suite now emits exact symbolic CCA artifacts
alongside the dark-mode plots.  For each analyzed checkpoint record, the
topology output directory contains:

```text
<record>_symbolic_resolution_certificate.json
<record>_symbolic_resolution_certificate_faces.csv
<record>_symbolic_resolution_certificate_minimal_nonfaces.csv
<record>_symbolic_resolution_certificate_hochster_betti_rows.csv
<record>_symbolic_resolution_certificate_taylor_multidegrees.csv
<record>_symbolic_resolution_certificate_taylor_ranks.csv
```

The JSON certificate records the finite object

```text
(V, Delta, I_Delta, beta_{i,sigma}, T(I_Delta), A_T)
```

where `V` is the chamber vertex set, `Delta` is the cyclic flag fan,
`I_Delta` is the Stanley-Reisner ideal generated by minimal nonfaces,
`beta_{i,sigma}` are the exact Hochster multigraded Betti numbers over
`F_2`, `T(I_Delta)` is the full Taylor resolution summarized by exact
multidegree counts, and `A_T` is the Taylor DG-algebra certificate.  The
DG-algebra record states the Taylor exterior basis, product rule, Taylor
differential, augmentation-ideal generator count, and the structural facts
...
The CSV files are intended for periodic review and regression tests: they make
the actual combinatorial objects inspectable without reverse-engineering a
heatmap.  This matters for BPB optimization because the train-time scalar
losses must remain tied to finite, falsifiable algebraic certificates rather
than opaque proxy plots.  If BPB stalls while the certificate metrics are
healthy, mixture/LR should be adjusted before increasing CCA weights.  If BPB
and certificate metrics degrade together, the next retune should lower the
...
120.  The repair is not to disable the symbolic CCA, DG, Koszul, or other
advanced losses.  The repair is to enforce the invariant

```text
finite(L_CE) and finite(L_aux) and finite(||grad||)
```

before an optimizer update is allowed to alter either weights or optimizer
state.  If this invariant fails, the step logs `train/nonfinite_update_skip=1`,
zeros gradients, and continues to the next batch without stepping Muon/Adam.
This preserves the mathematical losses while preventing a single nonfinite
minibatch from poisoning all later BPB measurements.

The advanced families remain enabled with nonzero weights.  PolarQuant remains
on by default, but train perturbation is ramped so early optimizer dynamics do
not combine full quantization noise, random-order decoding, and the full
step-0 CCA/DG stack before gradients have stabilized.
