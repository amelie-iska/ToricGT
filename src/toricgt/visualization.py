...
def plot_energy_landscape(path: np.ndarray, energy: np.ndarray, output: str | Path) -> None:
    ...
    fig, ax = plt.subplots(figsize=(7.5, 6), facecolor="#05070d")
    ax.set_facecolor("#05070d")
    levels = ax.contourf(xx, yy, landscape, levels=32, cmap="magma")
    ax.plot(path[:, 0], path[:, 1], color="#50f5ff", linewidth=1.6)
    ax.scatter(path[:, 0], path[:, 1], c=energy, cmap="viridis_r", s=14, edgecolor="none")
    best = int(np.argmin(energy))
    ax.scatter(path[best, 0], path[best, 1], s=120, color="#8cff6a", edgecolor="white")
    ...
    ax.yaxis.label.set_color("white")
    ax.title.set_color("white")
    cbar = fig.colorbar(levels, ax=ax)
